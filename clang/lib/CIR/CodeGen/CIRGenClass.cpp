...
void CIRGenFunction::emitImplicitAssignmentOperatorBody(FunctionArgList &args) {
  ...
  // Emit a memcpy from the source to the destination before processing the body
  // of defaulted union copy/move assignment operators.
  if (assignOp->getParent()->isUnion() && assignOp->isDefaulted()) {
    Address thisAddr = loadCXXThisAddress();
    ...
    mlir::Value srcPtr =
        builder.createLoad(getLoc(assignOp->getLocation()), srcPtrAddr);

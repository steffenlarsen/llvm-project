...
CIRGenOffloadRuntime::emitCUDAKernelCallExpr(CIRGenFunction &cgf,
  ...
  {
    // Check if the kernel has a body in this TU.  For template instantiations,
    // the body is on the template pattern, not the instantiation's redecl chain.
    bool hasDef = kernelDecl->hasBody();
    if (!hasDef)
      if (auto *pattern = kernelDecl->getTemplateInstantiationPattern())
        hasDef = pattern->hasBody();
    if (!hasDef) {
      // Cross-TU kernel: declared but not defined in this TU.  Fall back to
      // the device stub path so the launch uses the same handle that the
      // defining TU registered.
      return CIRGenCUDARuntime::emitCUDAKernelCallExpr(cgf, expr, retValue);
    }
  }
  ...
  cir::OffloadKernelLaunchOp::create(builder, loc, callee, gridX, gridY, gridZ,
...
CIRGenOffloadRuntime::emitCUDARuntimeCall(CIRGenFunction &cgf,
  ...
  if (name == "hipMalloc" || name == "hipHostMalloc" ||
    ...
    auto fnTy = cir::FuncType::get(argTys, hipErrTy);
    mlir::Value ret = emitHIPCall(cgf, loc, name, fnTy, args);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (ret && ret.getType() == retTy)
      return RValue::get(ret);
    return RValue::get(makeHipSuccess(builder, loc, retTy));

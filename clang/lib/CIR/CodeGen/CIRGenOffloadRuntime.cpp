...
static mlir::Value emitDim3Component(CIRGenFunction &cgf, mlir::Location loc,
  ...
  if (!rd)
    return builder.getConstantInt(loc, builder.getUInt32Ty(), 1);
  ...
  return cgf.emitLoadOfScalar(fieldLV, SourceLocation());
}
...
CIRGenOffloadRuntime::emitCUDAKernelCallExpr(CIRGenFunction &cgf,
  ...
  // No gpu.container_module attribute needed — cir.offload.kernel_launch
  ...
  // Note: we do NOT check hasBody() here.  For forward-declared kernels
  // and template instantiations, the definition may not have been parsed
  // yet at this point, but it will appear in the offload module by the
  // time CIR is finalized.  Falling back to the stub path for these
  // cases produces incorrect code (the stub path is broken for kernels
  // that go through the offload module).
  ...
  // We extract the x, y, z fields as CIR integer values directly.
  // ------------------------------------------------------------------ //
  const CallExpr *config = expr->getConfig();
  mlir::Value gridX, gridY, gridZ, blockX, blockY, blockZ;
  mlir::Value shmemVal, streamVal;
  ...
  if (config && config->getNumArgs() >= 2) {
    ...
    if (config->getNumArgs() >= 3) {
      ...
      if (!isZeroShmem)
        shmemVal = cgf.emitScalarExpr(shmemArg);
    }
    ...
    if (config->getNumArgs() >= 4) {
      ...
      if (streamArg->isNullPointerConstant(cgf.getContext(),
              Expr::NPC_ValueDependentIsNotNull))
        isNullStream = true;
      if (!isNullStream)
        streamVal = cgf.emitScalarExpr(streamArg);
    }
  } else {
    auto one = [&]() {
      return builder.getConstantInt(loc, builder.getUInt32Ty(), 1);
  ...
  // ------------------------------------------------------------------ //
  // 4. Emit cir.offload.kernel_launch.
  //
  // Kernel refs array: [@offload_device_module::@kernelName].
  // The merge pass rewrites this to per-arch module refs.
  // ------------------------------------------------------------------ //
  mlir::SymbolRefAttr callee =
      mlir::SymbolRefAttr::get(ctx, "offload_device_module",
                               {mlir::FlatSymbolRefAttr::get(ctx, kernelName)});
  mlir::ArrayAttr kernels = mlir::ArrayAttr::get(ctx, {callee});

  cir::OffloadKernelLaunchOp::create(builder, loc, kernels, gridX, gridY, gridZ,
                                     blockX, blockY, blockZ, shmemVal,
                                     mlir::ValueRange(args), streamVal);
...
CIRGenOffloadRuntime::emitCUDARuntimeCall(CIRGenFunction &cgf,
  ...
  // hipDeviceSynchronize() → cir.offload.device_sync
  if (name == "hipDeviceSynchronize") {
    auto hipErrTy = cir::IntType::get(ctx, 32, /*isSigned=*/true);
    auto op = cir::OffloadDeviceSyncOp::create(builder, loc, hipErrTy);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
  ...
  // ------------------------------------------------------------------ //
  // hipMalloc / hipHostMalloc / hipMallocManaged → cir.offload.malloc
  if (name == "hipMalloc" || name == "hipHostMalloc" ||
      name == "hipMallocManaged") {
    SmallVector<mlir::Value> args;
    for (unsigned i = 0; i < e->getNumArgs(); ++i)
      args.push_back(cgf.emitScalarExpr(e->getArg(i)));
    auto voidPtrPtrTy = cir::PointerType::get(
        cir::PointerType::get(cir::VoidType::get(ctx)));
    if (args[0].getType() != voidPtrPtrTy)
      args[0] = cgf.getBuilder().createBitcast(args[0], voidPtrPtrTy);
    auto hipErrTy = cir::IntType::get(ctx, 32, /*isSigned=*/true);
    auto kind = name == "hipMallocManaged"
                    ? cir::OffloadAllocKind::Managed
                    : name == "hipHostMalloc"
                          ? cir::OffloadAllocKind::Host
                          : cir::OffloadAllocKind::Device;
    auto op = cir::OffloadMallocOp::create(
        builder, loc, hipErrTy, args[0], args[1], kind);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
    return RValue::get(makeHipSuccess(builder, loc, retTy));
  ...
  // hipFree / hipHostFree → cir.offload.free
  if (name == "hipFree" || name == "hipHostFree") {
    mlir::Value ptr = cgf.emitScalarExpr(e->getArg(0));
    auto hipErrTy = cir::IntType::get(ctx, 32, /*isSigned=*/true);
    auto op = cir::OffloadFreeOp::create(builder, loc, hipErrTy, ptr);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
    return RValue::get(makeHipSuccess(builder, loc, retTy));
  ...
  // hipMemcpy / hipMemcpyAsync → cir.offload.memcpy
  if (name == "hipMemcpy" || name == "hipMemcpyAsync") {
    SmallVector<mlir::Value> args;
    for (unsigned i = 0; i < e->getNumArgs(); ++i)
      args.push_back(cgf.emitScalarExpr(e->getArg(i)));
    auto hipErrTy = cir::IntType::get(ctx, 32, /*isSigned=*/true);
    // hipMemcpy(dst, src, size, direction [, stream])
    mlir::Value stream;
    if (args.size() >= 5)
      stream = args[4];
    auto op = cir::OffloadMemcpyOp::create(
        builder, loc, hipErrTy, args[0], args[1], args[2], args[3], stream);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
    return RValue::get(makeHipSuccess(builder, loc, retTy));
  ...
  if (name == "hipMemcpyToSymbol" || name == "hipMemcpyFromSymbol") {
    SmallVector<mlir::Value> args;
    // The first argument is the device symbol — emit as an address (pointer
    // to the host-side shadow global), not as a scalar load. The HIP runtime
    // uses this address as a lookup key in the __hipRegisterVar table.
    auto symbolLVal = cgf.emitLValue(e->getArg(0));
    auto voidPtrTy = cir::PointerType::get(cir::VoidType::get(ctx));
    args.push_back(cgf.getBuilder().createBitcast(
        symbolLVal.getPointer(), voidPtrTy));
    for (unsigned i = 1; i < e->getNumArgs(); ++i)
      args.push_back(cgf.emitScalarExpr(e->getArg(i)));
    return emitHIPRuntimeCall(name, args);
  }

  // hipMemset → cir.offload.memset (D16/D32 variants stay as cir.call)
  if (name == "hipMemset") {
    SmallVector<mlir::Value> args;
    for (unsigned i = 0; i < e->getNumArgs(); ++i)
      args.push_back(cgf.emitScalarExpr(e->getArg(i)));
    auto hipErrTy = cir::IntType::get(ctx, 32, /*isSigned=*/true);
    // hipMemset(dst, value, size)
    mlir::Value stream;
    auto op = cir::OffloadMemsetOp::create(
        builder, loc, hipErrTy, args[0], args[1], args[2], stream);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
    return RValue::get(makeHipSuccess(builder, loc, retTy));
  ...
  if (name == "hipMemsetD16Async" || name == "hipMemsetD32Async") {
    SmallVector<mlir::Value> args;
    for (unsigned i = 0; i < e->getNumArgs(); ++i)
      args.push_back(cgf.emitScalarExpr(e->getArg(i)));
    return emitHIPRuntimeCall(name, args);
  ...
  // hipStreamSynchronize → cir.offload.stream_sync
  if (name == "hipStreamSynchronize") {
    mlir::Value stream = cgf.emitScalarExpr(e->getArg(0));
    auto op = cir::OffloadStreamSyncOp::create(builder, loc, hipI32Ty, stream);
    mlir::Type retTy = cgf.convertType(e->getType());
    if (op.getErr().getType() == retTy)
      return RValue::get(op.getErr());
    return RValue::get(makeHipSuccess(builder, loc, retTy));

  ...
  void HandleTranslationUnit(ASTContext &C) override {
    ...
    if (!FEOptions.ClangIRDisablePasses) {
      ...
      if (runCIRToCIRPasses(
              MlirModule, MlirCtx, C, enableVerifier,
      ...
      // Note: CXXABILowering now handles cir.offload.func natively via
      // OffloadFuncOpABILowering, so no stray gpu.return fixup is needed.
    }
...
void CIRGenAction::ExecuteAction() {
  ...
  if (getCurrentFileKind().getLanguage() == clang::Language::CIR) {
    ...
    // Note: stray gpu.return fixup no longer needed — CXXABILowering handles
    // cir.offload.func natively and the offload→GPU conversion runs later.
    ...
    offloadConfig.finiteOnly =
        MlirModule->hasAttr("cir.finite_math_only");
    offloadConfig.daz = offloadConfig.unsafeMathOpt;
    offloadConfig.fpContractFast =
        MlirModule->hasAttr("cir.fp_contract_fast");
    std::unique_ptr<llvm::Module> LLVMModule = lowerFromCIRToLLVMIR(
        MlirModule, LLVMCtx, CI.getLangOpts().OpenMP, /*mlirSaveTemps=*/"",
        /*fs=*/nullptr, CGO.ClangIROffload, offloadArchs,

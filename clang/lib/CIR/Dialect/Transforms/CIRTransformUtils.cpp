...
cir::ValueTraceResult cir::traceValueOrigin(mlir::Value v, unsigned maxDepth) {
  // A null value has no origin. Callers reach this with one whenever an
  // analysis declined to trace something -- an un-recovered launch dimension,
  // say -- and the matchers below assert rather than tolerate it.
  if (!v)
    return {};
    ...
    for (mlir::Block &b : *funcRegion) {
      for (mlir::Operation &op : b) {
        ...
        for (mlir::NamedAttribute na : op.getAttrDictionary()) {
          if (auto sym =
                  mlir::dyn_cast<mlir::FlatSymbolRefAttr>(na.getValue())) {
            calleeAttr = sym;
            break;
          }
        }
...
std::optional<int64_t> cir::matchCeilDiv(mlir::Value v, mlir::Value &dividend) {
  // Strip casts.
...
static mlir::Value getYieldedValue(mlir::Region &region) {
  if (!region.hasOneBlock())
    return {};
  auto yield =
      llvm::dyn_cast_or_null<cir::YieldOp>(region.front().getTerminator());
...
static void
pushThroughReferenceReturningCall(mlir::Value v,
                                  llvm::function_ref<void(mlir::Value)> push) {
...
cir::findGridDimCandidate(mlir::Value gridDim, mlir::ValueRange kernelArgs) {
  ...
  while (!worklist.empty()) {
    ...
    if (std::optional<unsigned> argIdx = traceToKernelArgIndex(v, kernelArgs)) {
      GridDimRelation rel;
      rel.argIndex = *argIdx;
...
cir::traceToKernelArgIndex(mlir::Value v, mlir::ValueRange kernelArgs) {
  auto result = traceValueOrigin(v);
...
cir::getAllocaAddrSpace(mlir::Operation *scope) {

...
private:
  ...
  void amendAMDGPUAtomicMetadata(
    ...
    // !atomic.ignore.denormal.mode is a fixed metadata kind (unlike the two
    // amdgpu.no.* markers below, which remain plain string-named metadata),
    // so it has to be attached via its enum rather than by name.

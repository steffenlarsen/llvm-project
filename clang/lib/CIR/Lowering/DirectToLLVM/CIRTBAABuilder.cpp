...
LLVM::TBAATagAttr CIRTBAABuilder::getStructPathTag(LLVM::GEPOp gep,
  ...
  // Verify the access type matches what the struct descriptor has at this
  // offset. If the struct descriptor used charType as a fallback for a
  // non-scalar field (e.g., a sub-struct), the access type won't appear
  // in the struct path and the LLVM verifier will reject it.
  auto structMembers = structDesc.getMembers();
  if (memberIdx < structMembers.size()) {
    auto expectedMember = structMembers[memberIdx];
    if (expectedMember.getTypeDesc() != memberDesc)
      return nullptr;
  }

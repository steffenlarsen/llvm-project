...
/// AMDGPUToolChain::addClangTargetOptions's choice of -mlink-builtin-bitcode
/// (internalize) vs -mlink-bitcode-file (don't) per
/// ToolChain::BitCodeLibraryInfo::ShouldInternalize: real ROCm device-lib
/// lists are not uniformly internalize=true (the ASan device runtime is
/// added with ShouldInternalize=false), so hardcoding true here would
/// mismatch that library's real behavior. The shared
/// CodeGenOpts.LinkBitcodeFiles list the primary compile draws from is not
/// a valid source for an aux entry's own arch-specific set.
...
/// one entered at construction. LangOptions::CUDAIsDevice is a separate
/// problem TargetScope does not cover: it's a single process-wide flag
/// CodeGen reads to choose host/device paths, and TargetScope deliberately
/// leaves it alone (see its own header comment). This swap lasts only for
/// the duration of one call into the aux CodeGenModule, not the whole AST
/// walk.
...
void BackendConsumer::Initialize(ASTContext &Ctx) {
  ...
  // source of truth here: it also includes the implicit "other side of
  // CUDA/OpenMP/SYCL" aux target populated from -aux-triple (no
  // -multi-target-aux-target flag needed), giving exactly one AuxGen for
  // ordinary two-pass compiles.
  ...
  if (HasAuxGens) {
    ...
    for (unsigned I = 0; I != NumAuxTargets; ++I) {
      ...
      // This entry's own values for the driver-decided, triple-dependent
      // LangOptions/CodeGenOptions flags a real, separately scheduled cc1
      // job for this target would compute from its own argv (see
      // BackendConsumer::AuxGenEntry's doc comment). Not "the other side of
      // the primary": two device-arch aux entries alongside a device
      // primary are both devices, so this is computed from Entry.TI's own
      // triple.
...
void BackendConsumer::runAuxBackendTail(AuxGenEntry &Entry) {
  ...
  MultiTargetCodeGenScope Scope(*Context, CI.getLangOpts(), CI.getCodeGenOpts(),
                                Entry);
  ...
  emitBackendOutput(CI, CI.getCodeGenOpts(), Entry.TI->getDataLayoutString(), M,
                    Entry.Action, FS, std::move(Entry.AsmOutStream), this,
...
void BackendConsumer::HandleTranslationUnit(ASTContext &C) {
  ...
  // that doesn't pass it takes the textually-unchanged branch below.
  ...
  // Package every configured aux entry's own bitcode into a real .hipfb,
  // once every runAuxBackendTail call above has completed. Off by default.
  if (ReorderForPackaging) {

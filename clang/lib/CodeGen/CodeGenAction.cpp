...
void BackendConsumer::Initialize(ASTContext &Ctx) {
  ...
  bool HasAuxGens = MultiTargetCodeGen && NumAuxTargets > 0;
  {
    ...
    // alternate pair that Sema deliberately leaves unmerged, such as one
    // driven by PendingBatchAmbientBuiltinTriggered -- and even a single,
    // no-aux-CodeGenModule CGM must still recognize itself as variant 1 to
    // emit that Decl, matching what a real, separately-invoked compile for
    // this target would produce. Neither condition fires for an ordinary
    ...
    // so this still can't give ordinary compiles a nonzero ambient variant
    // for the first time.

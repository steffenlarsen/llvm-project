...
public:
  ...
  /// scenario breaks, see
  /// stage7-step5-samesid-auxtarget-macro-collision-fixed.md). Each entry
  /// drives its own second CodeGenModule pipeline off the same shared AST as
  /// Gen, selecting its own Decl::TargetVariant. Stops at producing a correct
  /// in-memory llvm::Module: unlike Gen, no entry's module is linked, embedded,
  /// or run through the backend yet -- that belongs to Stage 7 Phase 2/3.
  struct AuxGenEntry {

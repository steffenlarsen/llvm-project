...
/// them would span everything in between.
static std::vector<Conditional> collapse(const TargetRecording &R,
...
class Aligner {
  ...
  bool equalRange(ArrayRef<std::pair<unsigned, unsigned>> Windows) const {
  ...
  childrenOf(unsigned Which, unsigned Lo, unsigned Hi,
  ...
  void coalesce() {
    ...
    for (unsigned I = 0; I != Segments.size();) {
      ...
      auto agrees = [&] {
        std::array<int, 3> D0 =
            delta(0, Segments[I].Ranges[0].first, Segments[J].Ranges[0].second);
  ...
  std::vector<bool> declarationBoundaries(unsigned Which = 0) const {
    ...
    for (auto [I, T] : llvm::enumerate(Toks)) {
      ...
      // (e.g. `struct X { ... };`). A cut point also cannot fall inside an
      ...
      // valid boundary: otherwise, when a divergent region has nothing
      // cuttable between its closing pragma pop and end of file, that
      // stream's own eof token would end up inside the alternative;
      // TokenLexer::Lex delivers it verbatim, so the parser stops there and
      // silently drops every later alternative's declarations.
    ...
    for (unsigned I = 0; I != Segments.size();) {
      ...
      while (!Boundary[Hi[0]] && J != Segments.size()) {
        ...
        if (P < B) {
          ...
          for (unsigned K = 1; K != N; ++K)
            Rest.Ranges[K].first =
                Next.Ranges[K].first + (P - Next.Ranges[0].first);
          std::vector<std::pair<unsigned, unsigned>> Ranges(N);
          for (unsigned K = 0; K != N; ++K)
            Ranges[K] = {Lo[K], Hi[K]};
          Out.push_back({false, std::move(Ranges)});
  ...
  /// Walk \p Q back over a trailing `__attribute__ ( ( ... ) )`, if any.
  ///
  /// libstdc++ writes `namespace std _GLIBCXX_VISIBILITY(default) { ... }`,
  /// and that macro expands to exactly this between the namespace name and
  /// its brace. Left unhandled, the brace's backward scan hits the
  /// attribute's `)` instead of `namespace`/an identifier, misclassifies the
  /// namespace's own brace as a function body, and every declaration inside
  /// it -- the entire namespace, once widened -- becomes uncuttable.
  static unsigned skipTrailingGnuAttribute(ArrayRef<Token> Toks, unsigned Q) {
    if (!Q || !Toks[Q - 1].is(tok::r_paren))
      return Q;
    unsigned R = Q;
    int Depth = 0;
    do {
      if (Toks[R - 1].is(tok::r_paren))
        ++Depth;
      else if (Toks[R - 1].is(tok::l_paren))
        --Depth;
      --R;
    } while (R && Depth);
    if (Depth || !R || !Toks[R - 1].is(tok::kw___attribute))
      return Q;
    return R - 1;
  }
  ...
  /// each self-contained. A function body's contents are statements, and
  /// cutting between them produces "function definition is not allowed here".
  ///
  /// Namespaces and linkage specifications only. Class bodies are containers
  /// too but the parser handles member declarations elsewhere, so allowing them
  /// here would emit markers where nothing consumes them.
  static bool isContainerBrace(ArrayRef<Token> Toks, unsigned P) {
    if (P >= 2 && Toks[P - 1].is(tok::string_literal) &&
      ...
      return true;
    unsigned Q = skipTrailingGnuAttribute(Toks, P);
    while (Q &&
           (Toks[Q - 1].is(tok::identifier) || Toks[Q - 1].is(tok::coloncolon)))
      --Q;
  ...
  ///
  /// Such a brace does not end the declaration: `typedef struct { ... }
  /// fd_set;` and `struct X { ... } x;` both continue past it, and only the `;`
  /// ends them. A function body's brace does end one. Walking back over the
  /// name and any base-clause reaches the tag keyword; a `)` on the way means a
  /// parameter list, so it is a function.
  ...
  /// declaration inside it sits at depth 1. Requiring absolute depth 0 would
  /// make such a region a single 73,000-token chunk that nothing can be
  /// trimmed from.
public:
  std::vector<unsigned> chunkFor(unsigned Which, unsigned Lo,
                                 unsigned Hi) const {
...
private:
  std::vector<unsigned> chunk(unsigned Which, unsigned Lo, unsigned Hi) const {
    ...
    // declaration -- without it, one divergence in <cmath> duplicates the
    // whole 73,698-token namespace.
    ...
    for (unsigned P = Lo; P != Hi; ++P) {
      ...
      if (K == tok::l_brace) {
        char Kind =
            isContainerBrace(Toks, P) ? 1 : (isTagBrace(Toks, P) ? 2 : 0);
        Open.push_back(Kind);
        BodyOpen += Kind != 1;
      ...
      // `struct X { ... };` and `typedef struct { ... } fd_set;` both run on
      // to the `;`; cutting at the `}` would let trimming split the tail off
      // as a shared run of its own, orphaning it once every arm ends at the
      // `}`. The token that closes the last open pragma scope is a cut point
      // for the same reason declarationBoundaries treats it as one -- see the
      // comment there.
  ...
  /// which is what lets each alternative be parsed on its own -- but it drags
  /// in whole declarations that are identical in every target. Parsing those
  /// once per target declares them once per target, which is a redefinition,
  /// and it is also wasted work: on ggml the widened regions are 11-26% of the
  /// stream and almost all of it is shared text.
  ///
  /// So each alternative is re-split at declaration boundaries and the two
  /// sequences are diffed. Diffing *tokens* doesn't work here: it has no idea
  /// where a construct starts and cuts regions in half. Diffing
  /// *declarations* does not have that problem: a declaration is
  /// self-contained, so any run of them can be spliced.
  void trimAlternatives() {
    ...
    for (const MergedSegment &S : Segments) {
      ...
      auto chunkLen = [&](unsigned K, unsigned I) {
        return C[K][I + 1] - C[K][I];
      };
...
translateLoc(SourceManager &Primary, const SourceManager &Aux,
             SourceLocation Loc, llvm::DenseMap<const void *, FileID> &Cache) {
  if (Loc.isInvalid())
    return Loc;
  ...
  if (It->second.isInvalid())
    return SourceLocation();
  return Primary.getLocForStartOfFile(It->second)
...
/// bytes either way.
static void translateToken(Token &T, Preprocessor &PP, const SourceManager &Aux,
                           llvm::DenseMap<const void *, FileID> &Cache) {
  T.setLocation(
      translateLoc(PP.getSourceManager(), Aux, T.getLocation(), Cache));
  if (T.isAnnotation()) {
    T.setAnnotationEndLoc(translateLoc(PP.getSourceManager(), Aux,
                                       T.getAnnotationEndLoc(), Cache));
    return;
  }
  if (T.isLiteral())
    return;
...
std::vector<Token>
clang::buildMergedStream(Preprocessor &PP, ArrayRef<TargetRecording> Recordings,
  ...
  for (const MergedSegment &S : Segments) {
    ...
    Out.push_back(
        makeMarker(tok::annot_target_alt_begin, Loc, S.Ranges.size()));
...
void clang::reportDeclarationGranularity(raw_ostream &OS,
  ...
  for (const MergedSegment &S : Segments)
    if (!S.Shared) {
      ...
      Deepest = std::max(
          Deepest, Depth[std::min<unsigned>(S.Ranges[0].first, Toks.size())]);
    }
  for (auto [Lo, Hi] : Wide)
    Widened += Hi - Lo;
...
void clang::reportAlignment(raw_ostream &OS,
  ...
  for (const MergedSegment *S : ArrayRef(Big).take_front(8)) {
    ...
    for (unsigned W = 0; W != N; ++W) {
      if (!Recordings[W].SM ||
          S->Ranges[W].first >= Recordings[W].Tokens.size())
        continue;

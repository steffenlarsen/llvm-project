...
private:
  ...
  void trimAlternatives() {
    unsigned N = Recs.size();
    std::vector<MergedSegment> Out;
    // Recs[0] (the primary) frequently has a null SM -- ParseAST.cpp builds
    // it inline without setting one -- so debug printing must fall back to
    // whichever stream actually has one (an aux recording always does).
    unsigned DbgK = 0;
    bool HaveDbgSM = false;
    if (getenv("DEBUG_TEXFN_TRIM")) {
      for (unsigned K = 0; K != N; ++K)
        if (Recs[K].SM) {
          DbgK = K;
          HaveDbgSM = true;
    ...
    for (const MergedSegment &S : Segments) {
      if (HaveDbgSM) {
        std::string Loc0 =
            Recs[DbgK].Tokens[S.Ranges[DbgK].first].getLocation().printToString(
                *Recs[DbgK].SM);
        if (Loc0.find("texture_fetch_functions") != std::string::npos ||
            Loc0.find("amd_surface_functions") != std::string::npos ||
            Loc0.find("amd_hip_fp16") != std::string::npos) {
          llvm::errs() << "TRIM-ENTRY start=" << Loc0 << " Shared=" << S.Shared
                       << "\n";
        }
      }
      if (S.Shared) {
      ...
      if (HaveDbgSM) {
        ...
        if (Loc0.find("texture_fetch_functions") != std::string::npos ||
            Loc0.find("amd_surface_functions") != std::string::npos ||
            Loc0.find("amd_hip_fp16") != std::string::npos) {
          llvm::errs() << "TRIM-SEG start=" << Loc0 << "\n";
          for (unsigned K = 0; K != N; ++K) {
            if (!Recs[K].SM) {
              llvm::errs() << "  stream " << K << " size="
                           << (S.Ranges[K].second - S.Ranges[K].first)
                           << " chunks=" << NChunks[K] << " (no SM)\n";
              continue;
            }
            std::string LocK =
                Recs[K].Tokens[S.Ranges[K].first].getLocation().printToString(
                    *Recs[K].SM);
            llvm::errs() << "  stream " << K
                         << " size=" << (S.Ranges[K].second - S.Ranges[K].first)
                         << " chunks=" << NChunks[K] << " start=" << LocK
                         << " end="
                         << Recs[K]
                                .Tokens[S.Ranges[K].second - 1]
                                .getLocation()
                                .printToString(*Recs[K].SM)
                         << "\n";
          }
        }
      }
      ...
      if (HaveDbgSM) {
        ...
        if (Loc0.find("texture_fetch_functions") != std::string::npos ||
            Loc0.find("amd_surface_functions") != std::string::npos ||
            Loc0.find("amd_hip_fp16") != std::string::npos) {
          llvm::errs() << "TRIM-RESULT start=" << Loc0
                       << " Balanced=" << Balanced
                       << " numPieces=" << Split.size() << "\n";
          for (const MergedSegment &T : Split) {
            llvm::errs() << "  piece Shared=" << T.Shared;
            for (unsigned K = 0; K != N; ++K)
              llvm::errs() << " size" << K << "="
                           << (T.Ranges[K].second - T.Ranges[K].first);
            llvm::errs() << "\n";
          }
        }
      }

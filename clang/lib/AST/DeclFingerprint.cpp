...
unsigned clang::mergeEquivalentVariants(ASTContext &Ctx) {
  ...
  for (auto &Entry : ByKey) {
    ...
    if (!A)
      continue;
    // Every other target's copy is checked against the primary independently:
    // with two or more aux targets, one target's copy can be interchangeable
    // with the primary's while another's genuinely differs, and the ones that
    // differ must keep their own tag rather than being silently left out of
    // the loop entirely (which is what happened when this only ever looked
    // for variant 1 and 2: a third target's copy was neither unclaimed nor
    // marked redundant, so it stayed visible alongside the now-shared primary
    // -- two live declarations for one call, "is ambiguous" with both
    // candidates identical).
    bool AnyEquivalent = false;
    for (auto &VariantEntry : ByVariant) {
      ...
      // function or variable's subtree contains no such node -- a
      // FunctionTemplateDecl's specializations live in Common->Specializations,
      // never in DeclContext::decls() -- so hiding it is always safe once A/B
      // are confirmed interchangeable.

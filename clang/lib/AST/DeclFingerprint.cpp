...
/// Type-kind argument built against one of Design 1's two independent
/// re-parses of a declaration as equal to the "same" argument built against
/// the other.
///
/// TemplateArgument::structurallyEquals compares a Type-kind argument by its
/// raw QualType pointer (TemplateBase.cpp), the same non-canonical identity
/// FunctionTemplateSpecializationInfo::Profile's FoldingSet hashing relies
/// on. Design 1's second re-parse re-lexes and re-parses a declaration's
/// tokens from scratch, so it mints its own, independent
/// TemplateTypeParmDecl for a template parameter like `T` -- any type built
/// by substituting into it (even something as plain as `float`) is a
/// distinct, non-uniquable sugar node from the one the first parse's `T`
/// produces, even though both denote the same canonical type. Comparing by
/// QualType::getCanonicalType() strips that per-parse substitution sugar and
/// compares the one true canonical type instead, so the two independently-
/// parsed copies' specializations line up. See
...
/// an equivalent for.
///
/// A specialization instantiated against B while it was still visible (e.g.
/// a caller swept into Design 1's reparse net that called `B<Args>` before
/// this pass ever ran) lives only in B's own Common::Specializations
/// FoldingSet: markRedundantTree's recursion above walks B's DeclContext and
/// template parameters, but a FunctionTemplateDecl/VarTemplateDecl's implicit
/// specializations are not children in any DeclContext, so that recursion
/// never reaches them. Left alone, such a specialization stays visible
/// (TargetVariant 0) and end-of-TU processing (e.g. `mmf.cuh`'s
/// `extern template` declarations, which only ever look at A's copy) never
/// touches it either -- see mmf-extern-template-host-emission-bug. Comparing
/// template arguments directly, rather than through
...
/// was originally created still matching now.
...
/// branches on arch macros (e.g. mul_mat_f_cuda launching an arch-divergent
/// kernel) is swept into Design 1's reparse net, producing two
/// FunctionTemplateDecls with genuinely different bodies -- never
/// interchangeable, so the pairing loop below never reaches them otherwise.
/// Each keeps its own, entirely separate Common::Specializations FoldingSet.
/// A file's `extern template` declarations are ordinary, non-divergent code:
/// parsed once, they only ever tag the primary's copy. Every real call site
/// lives inside a caller that is itself swept into the same reparse net, so
/// it resolves to the *other* copy's FoldingSet instead, and its
/// specializations never see the extern-template tag -- they stay
/// TSK_ImplicitInstantiation and get fully defined at end-of-TU regardless of
/// what the file's own extern template declared. See
/// mmf-extern-template-host-emission-bug. Comparing template arguments
/// directly, as hideRedundantSpecializations does, avoids relying on the
/// ambient target variant that was active when each side's specialization
/// was originally created still matching now.
...
/// -- e.g. `tile_like<16,16,int> t` inside kernel_like
/// (multi-target-class-template-value-member.hip), where each target's Sema
/// pass instantiates its own, distinct ClassTemplateSpecializationDecl (see
...
static bool isInterchangeable(ASTContext &Ctx,
  ...
  // deeper for a class template than for a function template, and nothing
  // above catches it: StructuralEquivalenceContext's own RecordDecl
  // comparison walks base classes, friends, and *data fields*
  // (RecordDecl::field_iterator) only, never type-alias members, and never a
  // method's body. A struct built entirely out of member typedefs --
  // `mma_tile_sizes`'s `T_A_KQ`/`T_C_KQ`/... tile<> aliases -- has zero
  // fields, so that comparison passes vacuously between two arms whose
  // aliases actually resolve to differently-shaped tile<> partial
  // specializations; a struct like `my_fp8` with a genuinely
  // `#if`/`#else`-divergent `operator float() const` body has zero fields
  // *and* zero divergent typedefs, so neither this function's own typedef
  // check nor StructuralEquivalenceContext's field-only walk ever inspects
  // the method body that actually differs. Both let it through as
  // "interchangeable" and merged the arms anyway.
  if (const auto *RA = dyn_cast<CXXRecordDecl>(A)) {
    ...
    // initializer is neither a field (StructuralEquivalenceContext's own walk)
    // nor a method body (the check just above) -- `traits<int>::value`'s
    // `#if defined(__gfx942__)`/`#else` initializer is the only thing that
    // differs between the two arms of an otherwise-identical specialization,
    // and nothing here inspected it, so both were declared interchangeable,
    // the aux arm's copy hidden, and the primary's own member silently
    // repointed to whichever arm completed its definition last.
...
unsigned clang::mergeEquivalentVariants(ASTContext &Ctx) {
  ...
  for (auto &Entry : ByKey) {
    ...
    // declaration seen for each variant, matching how A and B were picked
    // before this generalised to N.
    ...
    if (!A)
      continue;
    // Every other target's copy is checked against the primary, but A is only
    // promoted to shared -- and any B hidden -- once *every* present variant
    // agrees, not merely the first one checked: with two or more aux targets,
    // one target's copy can be interchangeable with the primary's while
    // another's genuinely differs (e.g. one aux arch shares the primary's
    // #else-branch body while a second aux arch's #if-gated body diverges).
    // Promoting A on partial agreement hid the disagreeing variant's own
    // correctly-tagged copy behind an already-"shared" A that had never
    // itself been queued for that variant's device codegen (A is the
    // primary's own declaration; a host-only pass never queues a
    // __device__-only function for codegen), leaving the majority-agreeing
    // variant's device module with a call and no body at all -- an undefined
    // symbol at link time, not a Sema-visible ambiguity. See
    // mergeequivalentvariants-partial-agreement-undefined-symbol.
    bool AllAgree = true;
    ...
    for (auto &VariantEntry : ByVariant) {
      ...
      StructuralEquivalenceContext SEC(Ctx.getLangOpts(), Ctx, Ctx, NonEquiv,
                                       ...
                                       /*Complain=*/false);
      // The copy B is left in place. Hiding it as well was tried: it removes
      // the 668-1,690 redundant duplicates but takes 633 declarations the
      // host needs with them (missing 126 -> 759), because the copy's subtree
      // is not the same shape as the original's. A duplicate is a lesser
      // defect than an absence.
    ...
    for (auto &VariantAndB : Agreeing) {
      ...
      // specialization reached only through its template's FoldingSet (see
      // the comment on the unpaired-A case above): hiding it is unsafe
      // without -hide-redundant-variants' documented 633-declaration loss. A
      // function or variable's own subtree contains no such node -- but a
      ...
      // below never hides one that was already instantiated against B before
      // this pass ran (see mmf-extern-template-host-emission-bug and
      // hideRedundantSpecializations above). Hiding B's own declaration is
      // always safe once A/B are confirmed interchangeable; hiding one of its
      // already-materialized specializations additionally needs A to already
      // have an equivalent, so a specialization A doesn't have isn't lost.
      //
      // EnumDecl is included here for the same reason as FunctionDecl/VarDecl,
      // not excluded like CXXRecordDecl/ClassTemplateDecl: an enum cannot be a
      // template and has no FoldingSet-reached specializations, so
      // markRedundantTree's DeclContext::decls() walk reaches every one of its
      // enumerators -- there is no hidden subtree this could lose. Without
      // this, an unscoped enum re-touched by a later, unrelated divergent
      // #if/#endif (see amd_hip_fp8.h's __hip_fp8_interpretation_t, spliced
      // right before ROCm's __vsubss4) gets a second EnumDecl copy that is
      // left tagged and visible after A is unclaimed to variant 0: both A's
      // now-shared enumerator and B's still-tagged one are simultaneously
      // visible whenever B's variant is current, so every reference to
      // __HIP_E4M3_FNUZ/__HIP_E4M3/__HIP_E5M2/__HIP_SATFINITE etc. is
      // "ambiguous" even though both candidates are the very same declaration
      // re-parsed. See mtcg-build-new-blockers-2026-09-16 item 2.
...
/// merged here (kernel_like in
/// multi-target-class-template-value-member.hip) permanently keeps whichever
/// target's copy happened to survive the merge for every other target too,
/// even though ClassTemplateSpecializationDecl::Profile already forked a
/// correct answer for each target before this merge discards all but one
/// reference to them.
///
...
void clang::mergeReparseAlternative(ASTContext &Ctx, ArrayRef<Decl *> Primary,
  ...
  for (size_t I = 0, E = Primary.size(); I != E; ++I) {
    ...
    markRedundantTree(B, Variant);
    // A is compared against every group seen so far for this reparse
    // (ParseAST.cpp), not just the true host origin, so a later call can see
    // an A that an earlier call already marked redundant (e.g. alternate 2
    // reconciled into the host, then alternate 3 compared against alternate
    // 2). Unclaiming would revert that already-redundant A's tag back to
    // shared, reviving a duplicate that must stay hidden -- only a live
    // (non-redundant) A is ever the thing to revert.
...
void clang::applyPendingReparseUnclaims(ArrayRef<Decl *> PendingUnclaim) {
  ...
  for (Decl *A : PendingUnclaim) {
    if (!Seen.insert(A).second)
      continue;
    // Unlike the call site's original single-primary form, a recorded A here
    // is not always the host origin: ParseAST.cpp also records this to
    // reconcile one already-returned alternate against another, so the
    // variant being reverted to shared has to be read off A itself rather
    // than assumed to be 1 (mirrors mergeWidenedAlternative's identical
    // reasoning below).
...
/// differ -- e.g. one arm's branch declares extra overloads or fp8
/// conversion helpers that the other arms' branches don't (confirmed for
/// real on amd_hip_fp8.h's HIP_FP8_CVT_FAST_PATH region: 259 declarations on
/// the host/gfx90a arms, 275 on gfx942's). Pairing by raw index breaks down
/// the moment such a divergence appears anywhere in the region: every
/// declaration after that point -- even ones that are otherwise byte-
/// identical across every arm, like an enum declared before any divergent
/// content -- goes unreconciled along with it, leaving it individually
/// tagged and colliding with its own already-reconciled, now-shared copy.
std::string widenedAlternativeKey(ASTContext &Ctx, const Decl *D,
  ...
  if (const auto *ND = dyn_cast<NamedDecl>(D)) {
    ...
    // that kind, matching the previous positional behaviour for these.
...
void clang::mergeWidenedAlternatives(ASTContext &Ctx,
  ...
  for (size_t I = 0, NArms = Arms.size(); I != NArms; ++I) {
    for (Decl *A : Arms[I].Decls) {
      ...
      // live copy alongside whichever decl it was originally folded into.
      if (A->getTargetVariant() == Decl::TargetVariantRedundant)
        continue;

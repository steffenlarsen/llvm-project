...
void ASTContext::cleanup() {
  ...
  for (auto I = DivergentRecordLayouts.begin(),
            E = DivergentRecordLayouts.end();
       I != E;)
    if (auto *R = const_cast<ASTRecordLayout *>((I++)->second))
      R->Destroy(*this);
...
/// Enables multi-target mode when an aux target is configured. Kept
/// file-local rather than declared in ASTContext.h, which is included almost
/// everywhere and should not grow a CommandLine.h dependency.
...
void ASTContext::InitBuiltinTypes(const TargetInfo &Target,
  ...
  // The only way into multi-target mode: all targets are known here and
  // nothing has been parsed yet, which is what TargetDivergence needs.
...
#include "clang/Basic/WebAssemblyReferenceTypes.def"
  ...
  if (Target.hasAMDGPUTypes() ||
      llvm::any_of(AuxTargets,
                   [](const TargetInfo *T) { return T->hasAMDGPUTypes(); })) {

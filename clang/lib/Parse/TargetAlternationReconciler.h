...
// The widening/reparse reconciliation state and logic, generalized so every
// nested declaration-sequence loop (namespace bodies, extern "C" blocks,
// class bodies) can feed the same bookkeeping. Parsing is single-threaded
// recursive
...
public:
  ...
  /// the accumulated reparse-candidate statistics; only ParseAST()'s
...
private:
  ...
  // (see recordDivergentValueRefs/TargetVariantDeclRedirects). Flushed at each
  ...
  // Genuine #if/#elif widening (as opposed to the reparse mechanism just

  ...
  if (HaveLexer) {
    ...
    if ((LLVM_UNLIKELY(RecordReplayTokens) || !AuxRecordings.empty() ||
      ...
      if (S.getASTContext().getNumAuxTargets() > 5) {
        ...
        // Kinds 0/1/2 below match the fixed set NumPragmaScopeKinds sizes
        // for: force_cuda_host_device, pack, attribute. Their payloads are
        // Sema/Parse types TokenStreamMerge (in clangLex) cannot depend on,
        // so this classifier is built here and passed down to it.
        ...
        std::vector<MergedSegment> Segs = alignRecordings(
            All, WidenAlternatives || ParseAllAlternatives,
            ClassifyPragmaScope);
    ...
    const bool Reparse =
        ReparseDivergentUsers && !MergedStream.empty() &&
        S.getASTContext().getNumAuxTargets() > 0;
    ...
    for (bool AtEOF = P.ParseFirstTopLevelDecl(ADecl, ImportState); !AtEOF;
      ...
      if (!Reparse ||
          !(S.TouchedDivergentEntity || S.TouchedAmbientTargetBuiltin) ||
          !ADecl || P.inTargetAlternative()) {
      ...
      // not apply when the only reason to reparse is an ambient-target
      // builtin (e.g. __builtin_amdgcn_processor_is): that answers
      // differently across device architectures, never across host vs.
      // device, so a __device__-only (or __global__) declaration can still
      // genuinely diverge along that axis.
      if (ByEntityLookup &&

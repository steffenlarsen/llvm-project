...
namespace {
/// One target besides the primary to record a pass for; carries its own
/// CPU/features/device-ness so each aux arch can be recorded independently.
struct RecordTarget {
  std::string Triple;
  std::string CPU;
  std::vector<std::string> Features;
  /// Explicit rather than inferred from the primary's CUDAIsDevice:
  /// CI.getMultiTargetAuxTargets() (see below) is always the AMDGPU/device
  /// side, regardless of which side the primary itself sits on, so this is
  /// unconditionally true for those entries -- see targetsToRecord() below.
  bool CUDAIsDevice;
};
} // namespace
...
/// those.
///
/// When -multi-target-aux-target configured N device archs, that list
/// (CompilerInstance::getMultiTargetAuxTargets()) mirrors
/// CompilerInstance::createASTContext()'s own branching on the same list, so
/// this function's count always matches ASTContext's AuxTargets count (same
/// order: element 0 is variant 2, element 1 is variant 3, etc).
///
/// CI.getMultiTargetAuxTargets() is always the AMDGPU/device side --
/// CompilerInstance::createASTContext() builds it from "AMDGPUSideTriple"
/// regardless of whether the *primary* job itself is device
/// (--cuda-device-only, where getTarget() is itself AMDGPU) or host (the
/// common -fintegrated-hip-device-codegen case, where the AMDGPU triple
/// lives on getAuxTarget() instead). So RT.CUDAIsDevice is unconditionally
/// true for these entries, not tied to the primary's own mode.
///
/// This never covers the host/device AuxTriple pairing (the CUDA/OpenMP/SYCL
...
/// archs. Skip it if it's already been redundantly named as one more
/// -multi-target-aux-target spec.
static std::vector<RecordTarget> targetsToRecord(CompilerInstance &CI) {
  ...
  if (!Aux.empty() && Aux != CI.getTargetOpts().Triple &&
      llvm::none_of(Targets,
                    [&](const RecordTarget &RT) { return RT.Triple == Aux; })) {
    RecordTarget RT;
    RT.Triple = Aux;
...
MultiTargetRecordings clang::recordTargetStreams(CompilerInstance &CI) {
  ...
  for (const RecordTarget &RT : targetsToRecord(CI)) {
    ...
    TO.Triple = RT.Triple;
    // The primary's own triple becomes this nested pass's aux triple only
    // when RT sits on the *opposite* side of CUDAIsDevice from the primary
    // (the host/device AuxTriple pairing): otherwise FrontendOpts.AuxTriple
    // stays whatever the primary's aux was, which after the swap above
    ...
    // (`(LangOpts.CUDA || LangOpts.isTargetDevice()) &&
    // PP.getAuxTargetInfo()`). Pointing this pass's aux at the primary's own
    // device triple/CPU would predefine the *primary's* arch-identity macro
    // (__gfx942__) alongside its own (__gfx1100__) -- both defined at once,
    // so an #if chain testing one before the other always picks the
    // primary's branch, collapsing every device-arch-vs-device-arch
    // recording to the primary's content. Left alone, this pass's aux stays
    ...
    // already resolved by targetsToRecord() -- from the
    // AuxTargetCPU/AuxTargetFeatures scalars for the single-aux case, or from
    // the aux TargetInfo's own TargetOpts for the N-aux-target case. TuneCPU
    // isn't part of either source, so it's always cleared.
    TO.TuneCPU.clear();
    TO.CPU = RT.CPU;
    ...
    // Offload compilations can put two targets on opposite sides of this
    // flag (host vs. single device) or on the same side (two device archs)
    // -- RT.CUDAIsDevice already encodes which, computed once in
    // targetsToRecord().

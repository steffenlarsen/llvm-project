...
MultiTargetRecordings clang::recordTargetStreams(CompilerInstance &CI) {
  ...
  for (const RecordTarget &RT : targetsToRecord(CI)) {
    ...
    // device triple/CPU would make it predefine the *primary's* arch-identity
    // macro (__gfx942__) alongside its own (__gfx1100__) -- both defined at
    // once, so an #if chain testing one before the other always picks the
    // primary's branch, making every device-arch-vs-device-arch recording
    // collapse to the primary's content. Confirmed via a real `#if
    // defined(__gfx942__)

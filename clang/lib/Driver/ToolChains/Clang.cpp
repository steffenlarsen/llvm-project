...
// embed it into this host cc1 job's module.
//
...
// index -- a mismatch would silently swap which CPU's bitcode lands in which
// fatbin slot rather than crash.
static void emitIntegratedHipDeviceCodegenFlags(const Driver &D, Compilation &C,
  ...
  // entries when this flag is set -- -multi-target-aux-target= alone only
  ...
  for (auto [Index, BA] : llvm::enumerate(Archs)) {
    ...
    // OffloadPackager::ConstructJob does above: getTargetFeatures with
    // IsAux=false, then drop the "-target-feature" marker entries it
    // interleaves. Must be IsAux=false (unlike the aux-triple block below):
    // IsAux=true emits "-aux-target-feature" instead, which the
    // starts_with("-target") filter would not catch.
    ...
    CmdArgs.push_back("-mllvm");
    CmdArgs.push_back(
        Args.MakeArgString("-multi-target-aux-target=" + Twine(Variant) + ":" +
                           CPU + ":" + llvm::join(Features, ",")));
    ...
    // Reuses the identical (ToolChain, DeviceArgs, BA) triple
    // AMDGPUToolChain::addClangTargetOptions's getDeviceLibs call would use
    // for a real per-arch cc1 job, so the resolved lib set matches exactly.
    // ShouldInternalize is per-file (e.g. the ASan device runtime uses
    // false, see RocmInstallationDetector::getCommonBitcodeLibs's
    // AddSanBCLibs), so encode it in the flag rather than assume true --
    // CodeGenAction.cpp's consumer needs it to pick -mlink-builtin-bitcode
    // vs -mlink-bitcode-file per file.

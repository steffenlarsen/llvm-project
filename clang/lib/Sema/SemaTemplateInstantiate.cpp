...
/// minted after the redundant tag was applied -- e.g. a still-live
/// reparse-origin caller performing its own deduction at end-of-TU drain
/// time. hideRedundantSpecializations-style reconciliation
/// (DeclFingerprint.cpp) only reconciles specializations that already exist
/// in the redundant template's FoldingSet at the single point
/// mergeEquivalentVariants runs; it cannot catch ones minted afterward. This
/// closes that gap lazily, at the point each new specialization would
/// otherwise be tagged with a real (colliding) target: if the surviving,
/// non-redundant sibling template already has an equivalent specialization
/// for these (fully-resolved, concrete) template arguments, this
/// specialization is itself just as redundant and must not be given a real
/// target tag.
...
/// Design 1's reparse-divergent-users mechanism can duplicate a member
/// (e.g. two "ne" VarDecls at the same location, one TargetVariant=0 and one
/// TargetVariantRedundant) as two distinct children of the *same* Pattern;
/// InstantiateClassImpl's member loop must skip the redundant one there, since
/// the live twin is also a child of Pattern and gets instantiated on its own
/// iteration. Increment 6's whole-class-body widening is different: it tags a
/// member TargetVariantRedundant on one arm's
/// ClassTemplatePartialSpecializationDecl while the live twin is unclaimed to
/// TargetVariant 0 on a *different* arm's separately-registered
/// ClassTemplatePartialSpecializationDecl (confirmed empirically: these arms
/// are never linked via Decl::redecls(), only via sharing one
/// ClassTemplateDecl's partial-specialization list) -- so for that case,
/// Pattern's own copy of the member is never duplicated *within* Pattern, and
...
bool Sema::InstantiateClassImpl(
  ...
  for (auto *Member : Pattern->decls()) {
    ...
    // Skip declarations that Design 1's reparse-divergent-users mechanism
    // widened and then marked as a redundant duplicate of a sibling already
    ...
    // the pattern itself, this is Increment 6's whole-class-body widening
    // instead (see hasLiveDuplicateInPattern): Pattern's own copy is its only
    // physical declaration of this member, merely tagged redundant because a
    // *different* arm's counterpart was chosen as the merge survivor, so
    // instantiate it normally rather than dropping it.
...
static ActionResult<CXXRecordDecl *> getPatternForClassTemplateSpecialization(
  ...
  if (!isa<ClassTemplatePartialSpecializationDecl *>(Specialized)) {
    ...
    // PROTOTYPE (Stage 4): see the comment in
    // usesPartialOrExplicitSpecialization above -- filter to the specialization
    // visible to the target actually being compiled before doing partial
    // ordering, or two targets' specializations of the same arguments look
    // ambiguously matched.
    if (LLVM_UNLIKELY(clang::AllowTargetVariantDecls)) {
  ...
  if (auto *PartialSpec =
    ...
    // PROTOTYPE (Stage 4): when the primary template's own redeclarations
    // are target-tagged (no partial spec matched -- see
    // ClassTemplateDecl::hasTargetTaggedPrimaryTemplate), pick the redecl
    // visible to the ambient actually being compiled, mirroring the
    // partial-spec filtering above. Prefer an exact tag match over one
    // admitted only by isVisibleForTarget's AllowFallback rule, for the
    // same reason the partial-spec block does (already-confirmed
    // regression precedent: an ambiguous/wrong arm can otherwise be
    // admitted when a genuine exact match exists among the redecls).
    // Falls back to today's unconditional Template->getTemplatedDecl()
    // when the predicate is false or nothing matches, so this is a no-op
    // for every template not shaped like mma_tile_sizes.
    if (LLVM_UNLIKELY(clang::AllowTargetVariantDecls) &&
      ...
      // empirically, same failure mode already fixed once for
      // isVisibleForTarget's unsafeAtomicAdd sibling lookup, see
      // SemaLookup.cpp): Template->redecls() only ever reported the one
      // arm the parser happened to enter first. The DeclContext's lookup
      // table is what actually holds every widened copy together.
  ...
  // PROTOTYPE (Stage 5, Phase 5): if the chosen pattern is itself tagged for
  // a real target variant (one arm of a widened #if/#elif partial spec),
  // stash that variant on the resulting specialization. A nested lookup
  // performed while instantiating this specialization's own members (e.g. a
  // static-data-member initializer referencing a second target-tagged
  // specialization) has no enclosing FunctionDecl to fall back on --
  // isVisibleForTarget's ambient fallback consults this table instead. See

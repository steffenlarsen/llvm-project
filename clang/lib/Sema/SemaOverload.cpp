...
Sema::AddArgumentDependentLookupCandidates(DeclarationName Name,
  ...
  // PROTOTYPE (Stage 4): ArgumentDependentLookup gathers candidates directly
  // from each associated namespace's own lookup table, bypassing
  // LookupResult::resolveKind()'s per-target filtering entirely (see
  // isVisibleForTarget's doc comment above ArgumentDependentLookup's
  // declaration) -- so a target-tagged reparse copy belonging to a
  // *different* target than the one currently being compiled would
  // otherwise be added here as an equally-good, and therefore ambiguous,
  // overload candidate alongside whichever copy ordinary lookup already
  // found. Drop those here, the same way resolveKind() already does for
  // ordinary lookup's own candidate set.

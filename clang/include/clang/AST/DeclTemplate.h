...
/// PROTOTYPE: true if \p D is a FunctionDecl whose real per-target siblings
/// genuinely diverge in body -- either a specialization of a
...
/// ordinary name lookup in its DeclContext. Shared between
/// SemaTemplateInstantiateDecl.cpp's DivergentCalleeVisitor (which looks for
/// this within an already-instantiated function body) and
/// ClassTemplateDecl::hasTargetTaggedValueMember() (which looks for this
/// within an uninstantiated static-data-member initializer).
LLVM_ABI bool isTargetDivergentCallee(const ValueDecl *D);

/// PROTOTYPE: true if \p D is a static data member of a class template
...
/// variant. Deliberately mirrors Profile()'s own disjunction so this neither
/// over- nor under-triggers relative to what Profile can actually fork. See
/// Sema::InstantiateDivergentCalleesInBody's FoundValueMembers loop, which
/// uses this to extend that mechanism (originally callee-only) to also cover
/// a shared caller merely reading e.g. tile_like<16,16,int>::ne.
LLVM_ABI bool isTargetDivergentValueMemberAccess(const ValueDecl *D);

/// PROTOTYPE: true if \p D is a CXXMethodDecl declared directly on a class
/// template specialization whose FoldingSet identity is target-forked by
...
/// (t.get_i(l)) are ordinary object-syntax member calls, never themselves
/// re-resolved after the first target's Sema pass binds them. See
/// Sema::InstantiateDivergentCalleesInBody's FoundMemberCallees loop, which
/// uses this to extend the per-target callee redirect mechanism (originally
/// FunctionDecl/FunctionTemplateDecl-only) to also cover a shared caller
/// invoking a method on a target-forked class template specialization.
LLVM_ABI bool isTargetDivergentMemberFunctionAccess(const ValueDecl *D);

/// PROTOTYPE: if \p NNS names a TypeAliasDecl (e.g.
/// mma_tile_sizes<DV,ncols>::T_B_KQ) declared on a specialization of a class
/// template whose primary-template redeclarations are target-tagged (see
...
/// T_B_KQ::I) was only ever instantiated once. Returns nullptr otherwise. See
/// Sema::InstantiateDivergentCalleesInBody's FoundAliasMembers loop, which
/// uses this to extend the value-member redirect mechanism above to a value
/// reached through such an alias, not just a value declared directly on a
/// target-tagged-primary-template specialization.
LLVM_ABI const TypeAliasDecl *
getTargetDivergentAliasQualifier(NestedNameSpecifier NNS);

/// PROTOTYPE: true if \p FD has a genuinely tagged (non-reparse-origin,
/// non-redundant) sibling FunctionDecl reachable via ordinary name lookup in
/// its own DeclContext -- i.e. FD's own text was itself #if/#elif-widened.
/// Used by isTargetDivergentCallee() above, and by callers of
/// DivergentValueCalleeRewriter (SemaTemplateInstantiateDecl.cpp) to tell
/// apart the two reasons a divergent callee can have no widened sibling to
/// redirect to: a real sibling exists but wasn't found (a bug to fix), versus
/// no sibling exists at all because FD's own text was never widened -- see
...
/// PROTOTYPE: true if any Stmt/Expr transitively reachable within \p S
/// invokes, or is a MemberExpr naming, a target-divergent callee (see
/// isTargetDivergentCallee above) -- see the definition in DeclTemplate.cpp
/// for the full walk. CodeGen's constant-folding fast paths
...
public:
  ...
  /// PROTOTYPE (Stage 4): true if this function template's redeclaration
  /// chain gives evidence its *body* (not signature -- see
  /// hasTargetTaggedParameterType() for that) diverges by target.
  ///
  ...
  /// without its own body actually differing between targets -- Design 1
  ...
  /// specialization (see mmf-extern-template-host-emission-bug). This can't
  /// be told apart from genuine divergence by counting or comparing tagged
  /// redecls -- how many exist at any given moment depends on when in the
  /// compile this is queried, since widening's other-target copies can be
  /// created later, on demand, by Sema::InstantiateDivergentCalleesInBody --
  /// so instead this trusts Decl::isTargetVariantReparseOrigin(), a flag
  /// recorded at the moment each redecl is tagged that is stable regardless
  /// of query timing: true only for Design 1's tagging.
  bool hasTargetTaggedRedeclaration() const;

  /// PROTOTYPE (Stage 4): true if this function template's own pattern body
  /// -- a single, never-`#if`-widened piece of source text -- directly
  /// references a target-divergent plain callee (e.g. process_tile<N>'s
...
public:
  ...
  /// PROTOTYPE (Stage 4): true if this template's own (primary) body
  /// declares a static data member whose initializer calls a
  /// target-divergent callee (see clang::isTargetDivergentCallee), e.g.
  ...
  /// PROTOTYPE (Stage 4): true if this template's own primary-template
  /// redeclarations are target-tagged, e.g. mma_tile_sizes<DV,ncols>
  /// (fattn-mma-f16.cuh), whose entire body is redeclared once per
  /// #if/#elif/#else arm with non-interchangeable members, unlike
  /// hasTargetTaggedValueMember()'s single-untagged-declaration shape.
  /// Implicit instantiations that resolve to the primary template (no
  /// partial specialization matched) only need a target-aware identity
  /// (see ClassTemplateSpecializationDecl::Profile) when this is true, the
  /// same way hasTargetTaggedPartialSpecialization() already gates it for

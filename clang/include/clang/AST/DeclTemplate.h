...
public:
  ...
  /// chain contains a declaration tagged for a specific target, i.e. it was
  /// actually widened into per-target alternatives during parsing (see
  /// Decl::getTargetVariant()). Unlike hasTargetTaggedParameterType(), which
  /// catches divergence carried by the *signature*, this catches a template
  /// whose *body* diverges by raw target macros (e.g. ggml_cuda_mma::mma's
  /// `#if defined(CDNA4)...#elif defined(CDNA2)...` overloads) -- the
  /// widening/reparse machinery already produced N correctly-tagged
  /// FunctionTemplateDecl redeclarations for such a template, but
  /// FunctionTemplateDecl::mergePrevDecl unifies their shared
  /// Common::Specializations FoldingSet, so without this check every
  /// target's call to the same specialization would find and reuse whichever
  /// target's body was instantiated first.

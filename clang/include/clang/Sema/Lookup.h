...
public:
  ...
  /// the only member of its frozen candidate set for this reference (see
  /// resolveKind()'s N==1 fast path) -- it relaxes a last-resort fallback
  /// that is otherwise unsafe whenever another, differently-tagged sibling
  /// could be simultaneously live in the same set.

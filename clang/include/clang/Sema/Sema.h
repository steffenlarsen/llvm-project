...
public:
  ...
  /// PROTOTYPE: set when shared code evaluates an ambient-target predicate
  /// builtin (e.g. __builtin_amdgcn_processor_is), which answers differently
  /// per target but touches no target-specific entity a lookup could flag.
  /// Drives the same per-target re-parse as TouchedDivergentEntity.
  bool TouchedAmbientTargetBuiltin = false;

  /// PROTOTYPE (Stage 7, Increment 8): set by
  /// Parser::HandleTargetAlternationMarker for as long as an
  /// annot_target_alt_begin/sep/end-delimited alternative's own tokens are
  /// being parsed (mirrors Parser::TargetAlternative != 0). Lets
  /// isVisibleForTarget's host-primary device-context fallback tell "Current ==
  /// 1 because we are genuinely inside alternative 1's own pushed TargetScope"
  /// (an exactly-tagged V == 1 declaration here is a deliberate, correct match
  /// -- e.g. a local union's fields declared inside that same alternative)
  /// apart from "Current == 1 only because nothing more specific ever set it"
  /// (ordinary shared code, where a V == 1 candidate is no more privileged than
  /// any other target's and the fallback must still redirect Current onto a
  /// real device variant) -- both look identical as a bare Current == 1
  /// comparison.
...
public:
  ...
  /// overload resolution (a call shape this project hasn't wired up yet,
  /// e.g. a member call), it is simply overwritten or left unused -- inert by
  /// default, never a correctness hazard, only a missed-fix opportunity.
  ...
  /// PROTOTYPE (Stage 5, Phase 5 Increment 3): \p Specialization is a
  /// just-instantiated, shared (target variant 0) function template
  ...
  /// records the result in ASTContext::TargetVariantCallees for
  ...
  /// PROTOTYPE (Stage 5, Phase 5, mmvq.cu launch_bounds fix Part 2): the
  /// CUDALaunchBoundsAttr analogue of InstantiateDivergentCalleesInBody

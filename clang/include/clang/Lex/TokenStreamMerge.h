...
/// A push (+1) or pop (-1) of one of a small, fixed set of independently
/// tracked pragma scopes, or no event (Kind == -1).
struct PragmaScopeEvent {
  int Kind = -1;
  int Delta = 0;
};

/// Number of independently tracked pragma-scope kinds (today:
/// force_cuda_host_device, pack, attribute, GCC visibility -- see
/// ParseAST.cpp's classifier).
inline constexpr unsigned NumPragmaScopeKinds = 4;

/// Classifies a token as opening/closing one of a small set of stateful,
/// paired pragma scopes (e.g. \c \#pragma \c pack(push)/pop) that a
/// recursive-descent parser tracks with a plain counter, and that must
/// therefore never be split across a Shared/Alt segment boundary -- doing so
/// desyncs the counter, since a Shared segment fires once while an Alt
/// segment fires once per target arm. Interpreting some of these pragmas'
/// payloads needs Sema/Parse types this library cannot depend on, so the
/// caller supplies this.
using PragmaScopeClassifier =
    llvm::function_ref<PragmaScopeEvent(const Token &)>;

/// A classifier that reports no pragma-scope events, for callers with no such
/// pragmas to protect.
inline PragmaScopeEvent noopPragmaScopeClassifier(const Token &) { return {}; }
...
/// \param Classify identifies paired pragma scopes that must not be split by
/// that widening or by the re-splitting that follows it.
...
LLVM_ABI std::vector<Token> selectTarget(ArrayRef<Token> Merged,
                                         unsigned Which);

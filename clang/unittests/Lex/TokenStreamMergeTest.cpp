...
//===----------------------------------------------------------------------===//
//
// N-way (N=3) coverage for TokenStreamMerge's alignRecordings/Aligner.
...
/// [First,End) belong to the conditional instance named \p Key. Recordings
/// are tied together by identical key spelling ("file:line:col" +
/// occurrence count). \p ID must be unique within \p R.
...
unsigned totalShared(ArrayRef<MergedSegment> Segments) {
  return llvm::count_if(Segments,
                        [](const MergedSegment &S) { return S.Shared; });
}
...
// different branch: the ordinary path, with no inactive recordings
// involved.
...
// inner key is not required to be present in every recording, which keeps
// the divergence scoped to the outer conditional's own (small) content
// instead of flattening the whole outer span into one divergent block.
...
// in recording 0 has total skew 1+5+5=11; a better one exists at a larger
// stream-0 offset with total skew 3+0+1=4. The content-hash search must
// pick the globally smallest total, not whichever candidate stream 0
// reaches first.
...
// bracket delta reaches {0,0,0}, not just recording 0's. Uses real
// l_brace/r_brace content (unlike the other tests here) so a version that
// only checks recording 0 could plausibly stop too early for the others.
TEST(TokenStreamMergeN3, BalanceAlternativesChecksEveryRecording) {
  ...
  // bracket delta must be balanced for every recording, independently of
  // recording 0.
  for (const MergedSegment &S : Segments) {
    if (S.Shared)
      continue;
...
// following the pop. A Shared segment fires once and an Alt segment fires
// once per target arm, so a push and its matching pop must never end up
// split across a segment boundary -- that would desync whichever counter a
// Parser/Sema uses to track the pragma's depth. hashToken ignores an
// annotation's payload, so a PragmaScopeClassifier is what lets the resync
// search tell push from pop instead of treating identical "pop + tail"
// content as a candidate Shared segment on its own.
TEST(TokenStreamMergeN3, PragmaScopeNotSplitAcrossBoundary) {
  ...
  auto ClassifyPack = [](const Token &T) -> PragmaScopeEvent {
    ...
    return {0, static_cast<int>(
                   reinterpret_cast<intptr_t>(T.getAnnotationValue()))};
  };
...
// Same shape as PragmaScopeNotSplitAcrossBoundary above, but modelled on
// `#pragma GCC visibility push(...)`/`pop` (annot_pragma_vis): push carries a
// non-null payload (the visibility type) and pop carries null, the opposite
// polarity of annot_pragma_pack's own encoding. Checks that this
// independently-tracked pragma-scope kind is handled like the others, and
// that NumPragmaScopeKinds' fixed-size PragmaDepth array does not misbehave
// at its highest valid index.
TEST(TokenStreamMergeN3, PragmaVisibilityScopeNotSplitAcrossBoundary) {
  TargetRecording A = freeform("a"), B = freeform("b"), C = freeform("c");
  for (TargetRecording *R : {&A, &B, &C})
    appendAnnot(*R, tok::annot_pragma_vis, /*Push(non-null)=*/1);
  unsigned PushIdxA = A.Tokens.size() - 1;
  ...
  for (TargetRecording *R : {&A, &B, &C})
    appendAnnot(*R, tok::annot_pragma_vis, /*Pop(null)=*/0);
  unsigned PopIdxA = A.Tokens.size() - 1;

  for (TargetRecording *R : {&A, &B, &C})
    appendChunk(*R, "9"); // identical tail: what a naive resync would carve out

  auto ClassifyVis = [](const Token &T) -> PragmaScopeEvent {
    if (!T.is(tok::annot_pragma_vis))
      return {};
  ...
  std::vector<TargetRecording> Recs{A, B, C};
  std::vector<MergedSegment> Segments =
      alignRecordings(Recs, /*WidenToDeclarations=*/true, ClassifyVis);

...
bool LowerTypeTestsModule::runForTesting(Module &M, ModuleAnalysisManager &AM) {
  ...
  if (!getClReadSummary(M).empty()) {
    ...
    if (ReadSummaryFile->getBuffer().starts_with("---")) {
      ...
      if (!Summary) {
        Err.print(getClReadSummary(M).c_str(), errs());
  ...
  bool Changed =
      LowerTypeTestsModule(
          M, AM,
          getClSummaryAction(M) == PassSummaryAction::Export ? Summary.get()
                                                             : nullptr,
          getClSummaryAction(M) == PassSummaryAction::Import ? Summary.get()
                                                             : nullptr)
          .lower();

...
bool GVNPass::isMemDepEnabled() const {
  ...
  // this for pass parameters, but the -enable-gvn-{memdep,memoryssa} option
...
PreservedAnalyses GVNPass::run(Function &F, FunctionAnalysisManager &AM) {
  ...
  // MemDep and MemorySSA are mutually exclusive; see isMemDepEnabled(). Honor
  // that here too by resolving MemSSAOn first and letting it override MemDep
  // when the two -enable-gvn-{memdep,memoryssa} flags disagree.
  bool MemSSAOn = Options.AllowMemorySSA.value_or(getGVNEnableMemorySSA(F));
  bool MemDepOn = MemSSAOn
                      ? Options.AllowMemDep.value_or(false)
                      : Options.AllowMemDep.value_or(getGVNEnableMemDep(F));
  auto *MemDep =
...
bool GVNPass::runImpl(Function &F, AssumptionCache &RunAC, DominatorTree &RunDT,
  ...
  if (areGVNMemDepAndMemorySSABothExplicitlyEnabled(F))

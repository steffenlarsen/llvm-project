...
// The command-line tokenizer and response-file expansion moved to
// CommandLineTokenizer.cpp so that clv2 and cl:: share one implementation.
// Their declarations still come from CommandLine.h.
...
void cl::PrintHelpMessage(bool Hidden, bool Categorized) {
  // Reachable without a prior cl::ParseCommandLineOptions -- a tool may parse
  // with clv2 and still call this.  Make sure the printers exist, and do
  // nothing if this process never ran a cl:: parse, since the printers walk
  // the active subcommand and there is none.
  initCommonOptions();
  if (!globalParser().getActiveSubCommand())
    return;
...
// PrintVersionMessage, SetVersionPrinter and AddExtraVersionPrinter live in
// CommandLineV2.cpp while both parsers coexist, so a printer registered
// through either one is seen by both.  cl::'s own -version handler still uses
// VersionPrinterInstance directly.

...
bool clv2::detail::validateRegexOption(StringRef Pattern, StringRef OptName,
  ...
  if (R.isValid(Error))
    return true;
  // Deliberately not rejectOptionValue(): existing tests assert this exact
  // wording, so it is reproduced verbatim.  The rejection still travels
  // through ParseDiag, so it honours the OnError policy rather than
...
bool clv2::detail::runParser(std::vector<OptionEntry> &GlobalEntries,
  ...
  // The snapshot outlives the drain scope: the hand-built list is appended
  // after it, and the registration index recorded in Frame.DynamicStorages
  // has to match what publishDynamicStorages later resolves.
...
CompiledParser OptionParser::compile() const {
  ...
  CP.Shareable = DynamicEntries.empty() && !DrainGlobalDynamic &&
                 llvm::none_of(Registries, [](const TypeErasedRegistry &R) {
...
// LLVMParseCommandLineOptions uses the cl:: implementation;
// LLVMParseCommandLineOptions2 above is the clv2 entry point.

...
static bool getProfileIsFSDiscriminator(const clv2::OptionsContext &Ctx) {
  return clv2::getOptValOrDefault<&clv2::PD_ProfileIsFS>(Ctx);
}

static bool getLazyLoadNameTable(const clv2::OptionsContext &Ctx) {
  return clv2::getOptValOrDefault<&clv2::PD_LazyLoadNameTable>(Ctx);
}
...
std::error_code SampleProfileReaderText::readImpl() {
  ...
  ProfileIsFS = getProfileIsFSDiscriminator(Ctx.getOptionsContext());
...
SampleProfileReaderExtBinaryBase::readStringBasedProfileSymbolList() {
  ...
  if (std::error_code EC =
          ProfSymList->read(Data, End - Data, Ctx.getOptionsContext()))
    return EC;
...
SampleProfileReaderExtBinaryBase::readNameTableSecLegacy(bool IsMD5,
  ...
  if (FixedLengthMD5) {
    ...
    if (getLazyLoadNameTable(Ctx.getOptionsContext())) {
...
std::error_code SampleProfileReaderGCC::readImpl() {
  assert(!getProfileIsFSDiscriminator(Ctx.getOptionsContext()) &&
...
void SampleProfileReader::computeSummary() {
  ...
  Summary =
      Builder.computeSummaryForProfiles(Profiles, Ctx.getOptionsContext());
}

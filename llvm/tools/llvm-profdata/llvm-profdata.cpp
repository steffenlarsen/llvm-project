...
inline constexpr clv2::OptionInfo<bool> ShowCompositeInfoOnlyOpt{
    "show-composite-info-only",
    "Show type IDs and payload sizes in a composite sample profile"};
...
inline constexpr clv2::SubCommandInfo<
    ...
    &ShowCompositeInfoOnlyOpt, &ShowBinaryIdsOpt, &ShowTemporalProfTracesOpt,
    &ShowCoveredOpt, &ShowProfileVersionOpt>
    ShowCmd{
        "show",
...
static void mergeSampleProfile(const WeightedFileVector &Inputs,
  ...
  auto WriterOrErr = SampleProfileWriter::create(
      Args.OutputFilename, FormatMap[Args.OutputFormat], OptsCtx);
...
static int showSampleProfile(const ShowArgs &Args, raw_fd_ostream &OS,
  ...
  if (Args.ShowSectionInfoOnly && Args.ShowCompositeInfoOnly)
...
int main(int argc, const char *argv[]) {
  ...
  if (Opts->isActive<&ShowCmd>()) {
    ...
    Args.ShowCompositeInfoOnly = S.get<&ShowCompositeInfoOnlyOpt>();

...
inline constexpr OptionInfo<bool> LLI_HostJITTriple{
    "host-jit-triple", "Print process triple and exit", Init{false}};
...
inline constexpr OptionsRegistry<
    ...
    &LLI_RemoteMCJIT, &LLI_ChildExecPath, &LLI_OptLevel, &LLI_HostJITTriple,
    &LLI_TargetTriple, &LLI_EntryFunc, &LLI_ExtraModules, &LLI_ExtraObjects,
    &LLI_ExtraArchives, &LLI_EnableCacheManager, &LLI_ObjectCacheDir,
    &LLI_FakeArgv0, &LLI_DisableCoreFiles, &LLI_NoLazyCompilation,
    &LLI_GenerateSoftFloatCalls, &LLI_NoProcessSymbols, &LLI_Platform,
    &LLI_OrcDumpKind,
    ...
    &CG_SwiftAsyncFramePointer, &CG_DontPlaceZerosInBSS,
    &CG_EnableGuaranteedTailCallOpt, &CG_DisableTailCalls,
    &CG_StackSymbolOrdering, &CG_StackRealign, &CG_TrapFuncName, &CG_UseCtors,
    &CG_DataSections, &CG_FunctionSections, &CG_IgnoreXCOFFVisibility,
    &CG_XCOFFTracebackTable, &CG_EnableBBAddrMap, &CG_BBSections, &CG_TLSSize,
    &CG_EmulatedTLS, &CG_EnableTLSDESC, &CG_UniqueSectionNames,
    &CG_UniqueBasicBlockSectionNames, &CG_SeparateNamedSections,
    &CG_EABIVersion, &CG_DebuggerTuning, &CG_VectorLibrary,
    &CG_EnableStackSizeSection, &CG_EnableAddrsig, &CG_EnableCallGraphSection,
    &CG_EmitCallSiteInfo, &CG_EnableMachineFunctionSplitter,
    &CG_EnableStaticDataPartitioning, &CG_EnableDebugEntryValues,
    &CG_ForceDwarfFrameSection, &CG_XRayFunctionIndex, &CG_DebugStrictDwarf,
    &CG_AlignLoops, &CG_JMCInstrument, &CG_XCOFFReadOnlyPointers, &CG_SaveStats>
    LLIToolReg;

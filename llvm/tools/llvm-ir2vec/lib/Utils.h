...
struct MIRContext {
  LLVMContext Context{
      llvm::clv2::defaultOptionsContext()}; // CRITICAL: Must be first for
                                            // proper destruction order
  std::unique_ptr<Module> M;
  std::unique_ptr<MachineModuleInfo> MMI;
  std::unique_ptr<TargetMachine> TM;

  /// Construct with an optional OptionsContext for passing parsed options.
  MIRContext(const clv2::OptionsContext &OptsCtx) : Context(OptsCtx) {}
};

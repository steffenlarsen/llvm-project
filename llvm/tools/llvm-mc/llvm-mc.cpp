...
static constexpr OptionInfo<bool> AsLexOpt{
    "as-lex", "Lex tokens from a .s file", ValueDisallowed,
    EnumGroup{"Action to perform:"}, Cat(MCCategory)};
static constexpr OptionInfo<bool> AssembleOpt{
    "assemble", "Assemble a .s file (default)", ValueDisallowed, EnumGroup{},
    Cat(MCCategory)};
static constexpr OptionInfo<bool> DisassembleOpt{
    "disassemble", "Disassemble strings of hex bytes", ValueDisallowed,
    EnumGroup{}, Cat(MCCategory)};
static constexpr OptionInfo<bool> MDisassembleOpt{
    "mdis", "Marked up disassembly of strings of hex bytes", ValueDisallowed,
    EnumGroup{}, Cat(MCCategory)};
static constexpr OptionInfo<bool> CDisassembleOpt{
    "cdis", "Colored disassembly of strings of hex bytes", ValueDisallowed,
    EnumGroup{}, Cat(MCCategory)};
...
static constexpr OptionsRegistry<
    ...
    &LexMotorolaIntegersOpt, &NoExecStackOpt, &ValidateCFIOpt, &AsLexOpt,
    &AssembleOpt, &DisassembleOpt, &MDisassembleOpt, &CDisassembleOpt,
...
int main(int argc, char **argv) {
  ...
  }
  {
    ...
    // wins.
    ActionType Action = AC_Assemble;
    unsigned ActionPos = 0;
    auto ConsiderAction = [&](bool Specified, unsigned Position,
                              ActionType Val) {
    ...
    ConsiderAction(O->specified<&AsLexOpt>(), O->position<&AsLexOpt>(),
                   AC_AsLex);
    ConsiderAction(O->specified<&AssembleOpt>(), O->position<&AssembleOpt>(),
                   AC_Assemble);
    ConsiderAction(O->specified<&DisassembleOpt>(),
                   O->position<&DisassembleOpt>(), AC_Disassemble);
    ConsiderAction(O->specified<&MDisassembleOpt>(),
                   O->position<&MDisassembleOpt>(), AC_MDisassemble);
    ConsiderAction(O->specified<&CDisassembleOpt>(),
                   O->position<&CDisassembleOpt>(), AC_CDisassemble);
    Args.Action = Action;
  }

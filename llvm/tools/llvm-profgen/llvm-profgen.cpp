...
inline constexpr OptionInfo<std::string> PerfScriptFilenameOpt{
    "perfscript",
    "Path of a trace created by the Linux `perf script` command. For "
...
inline constexpr OptionInfo<std::string> PerfDataFilenameOpt{
    "perfdata",
    "Path of raw perf data created by the Linux perf tool. For LBR or "
...
inline constexpr OptionInfo<bool> WarnNotSymbolizedOpt{
    "warn-not-symbolized", "Generate warnings for unsymbolized addresses",
    Init{false}, Cat(ProfGenCat)};
...
static constexpr llvm::clv2::OptionsRegistry<
    ...
    &KernelBinaryOpt, &WarnNotSymbolizedOpt, &EnableCSPreInlinerOpt,
    &UseContextCostForPreInlinerOpt, &SamplePreInlineReplayOpt,
    &CSPreinlMultiplierForPrevInlOpt, &MaximumSearchDepthOpt>
    ProfGenToolReg;
...
int main(int argc, const char *argv[]) {
  ...
  Config.WarnNotSymbolized = Opts->get<&WarnNotSymbolizedOpt>();

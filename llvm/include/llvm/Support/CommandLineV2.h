...
// OptionCategory
//
// Purely a compile-time metadata tag.
...
template <typename ValueType> class CommonOptionInfo {
public:
  llvm::StringRef CLIName;
  llvm::StringRef CLIDescription;
  ...
  llvm::StringRef ValueDesc; ///< Custom placeholder text for help output
  ...
  // Enum values table (null if not an enum option).  Shared by every
  // subclass, since each names it via its own ValueType.
  ...
  /// BitsOptionInfo never invokes this; the field just goes unused there.
...
template <typename T> struct BitsOptionInfo : CommonOptionInfo<T> {
  using Base = CommonOptionInfo<T>;
  using ValueT = T;
...
struct AliasInfo : AliasTag {
  ...
  /// Picked up by StorageTypeOf<D, void_t<D::StorageT>>
  struct NoStorage {};
...
public:
  ...
  // Cannot be made private: a registry containing subcommands stores
  // std::optional<ParsedOptions<...>> in its own storage, and std::optional's
  // copy constructor needs the copy to be accessible.  Prefer clone() at
...
// descriptor, living in .rodata with no dynamic initialiser.  OptionEntry
// points at it through its Static field; see OptionEntry::info() below.
//===----------------------------------------------------------------------===//
...
struct ParseFrame {
  ...
  /// Resolved from whether an error stream was supplied to parse().
  ...
  /// storage was already default-initialised by TypeErasedRegistry::MakeStorage,
...
template <auto *Opt> const OptionStaticInfo *standaloneEnumStatics() {
  ...
  static const std::vector<OptionStaticInfo> V = [] {
    ...
    for (std::size_t I = 0; I < Opt->NumEnumVals; ++I) {
      ...
      if (I == 0 && !Opt->CLIDescription.empty())
...
//
// Free templates rather than members of OptionsRegistry<Opts...>: none of
// them use that pack, so making them members would mangle the enclosing
// registry's full option pack into every instantiation's symbol name -- a
// large amount of symbol-table text for helpers that only need access to
// ParsedOptions internals via detail::POAccess.  As free templates they
// mangle against their own arguments only.
//===----------------------------------------------------------------------===//
...
void addOneEntry(SlotT &Slot, unsigned &Count, unsigned &Pos,
  ...
  if constexpr (std::is_enum_v<SlotT>) {
    ...
    if constexpr (std::is_same_v<InnerDescT, OptionInfo<SlotT>>) {
      if (P->CLIName.empty() && P->NumEnumVals > 0) {
...
public:
  ...
  /// ParsedOptions. Used by TypeErasedRegistry for type-erased entry building.
...
//===----------------------------------------------------------------------===//
// TypeErasedRegistry - type-erased registry for runtime composition
//===----------------------------------------------------------------------===//
...
/// Register a TypeErasedRegistry for automatic discovery during parsing.
...
constexpr OptionInfo<EnumT> makeEnumOption(llvm::StringRef Name,
                                           llvm::StringRef Desc,

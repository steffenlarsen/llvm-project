...
//===----------------------------------------------------------------------===//
// Helper: parse argv vector directly against a single registry (no
// OptionsContext), and return (optional<ParsedOptions>, error string).
//===----------------------------------------------------------------------===//

template <const auto *Reg> auto parseSingle(std::vector<const char *> Argv) {
  std::string ErrBuf;
  raw_string_ostream Errs(ErrBuf);
  auto Opts = Reg->parse(static_cast<int>(Argv.size()), Argv.data(),
                        "test-tool", &Errs);
  return std::make_pair(std::move(Opts), std::move(ErrBuf));
}
...
// Comparing a function-pointer template argument against nullptr directly
// makes GCC emit -Waddress ("the address of F will never be NULL") at every
// instantiation that supplies a real function, because at the point of
// comparison it sees a named function. Passing the pointer through a
// parameter keeps the test a constant expression without the false positive.
template <typename FnT> constexpr bool isNullFnPtr(FnT F) {
  return F == nullptr;
}

// Scalar: the bool ValueOptional promotion happens at compile time.
static_assert(OptVerbose.ValueExpected == ValueOptional);
static_assert(OptVerbose.SuppressValuePlaceholder);
static_assert(OptCount.ValueExpected == ValueRequired);
static_assert(!OptCount.SuppressValuePlaceholder);
static_assert(OptCount.Name.size() == 5); // "count"
// Desc itself stays null for an ordinary descriptor (see the comment on
// OptionStaticInfo::Desc); effectiveDesc() is what resolves to its own
// address.
static_assert(OptCount.Desc == nullptr);
static_assert(clv2::detail::effectiveDesc(OptCount) == &OptCount);
static_assert(OptCount.DefaultValueName.size() == 3); // "int"

// An explicit ValueOptional is respected rather than promoted.
static_assert(OptToggle.ValueExpected == ValueOptional);

// value_desc and Required survive the flattening.
static_assert(OptFileOutput.ValueDesc.size() == 8); // filename
static_assert(OptFileOutput.OccurrencesFlag == Required);

// Lists default to ZeroOrMore and ValueRequired.
static_assert(OptInputs.OccurrencesFlag == ZeroOrMore);
static_assert(OptInputs.ValueExpected == ValueRequired);

// Positional-ness and prefix formatting.
static_assert(OptRest.IsPositional);
static_assert(PrefixOpt.IsPrefix);
static_assert(!PrefixOpt.IsAlwaysPrefix);
...
static_assert(OptOptim.NumEnumVals == 4);
...
static_assert(!isNullFnPtr(OptOptim.PrintEnumVal));

// Bits options are always repeatable, named and value-taking.
static_assert(BitsOpt.OccurrencesFlag == ZeroOrMore);
static_assert(BitsOpt.ValueExpected == ValueRequired);
static_assert(BitsOpt.NumEnumVals == 3);
...
static_assert(ReallyHiddenOpt.DefaultHidden == ReallyHidden);
} // namespace
...
TEST(CommandLineV2, RuntimeSubcommandRegistryConcurrentAppends) {
  ...
  // Every registration survived: a parse resolves each one as an active
  // subcommand rather than erroring.
  auto ParseSubcommand = [](StringRef Name) {
    OptionParser P;
    std::string NameStorage = Name.str();
    const char *Argv[] = {"tool", NameStorage.c_str()};
    std::string Err;
    raw_string_ostream OS(Err);
    auto Ctx = P.parse(2, Argv, "tool", &OS);
    EXPECT_NE(Ctx, nullptr) << Err;
    if (Ctx)
      EXPECT_EQ(Ctx->getActiveSubCommand(), Name);
  };
  ParseSubcommand("rt-sub-0-0");
  ParseSubcommand("rt-sub-3-39");
  ParseSubcommand("rt-sub-1-7");
}
...
TEST(CommandLineV2, CApiReturnsNullOnBadArgumentInsteadOfExiting) {
  ...
  LLVMOptionsContextRef Ctx = LLVMParseCommandLineOptionsV2(2, Argv, "");
  EXPECT_EQ(Ctx, nullptr);
...
TEST(CommandLineV2, CApiReturnsContextOnSuccess) {
  ...
  LLVMOptionsContextRef Ctx = LLVMParseCommandLineOptionsV2(1, Argv, "");
...
TEST(CommandLineV2, OnErrorReturnDoesNotExit) {
  // With no error stream the behaviour is std::exit(), which is fatal for a
  // parser embedded in a long-running process.  Supplying an error stream
  // must make a bad command line -- and --help -- return instead of exiting.
  // If this regresses the test process dies, which gtest reports.
  OptionParser P;
  P.add<&CmpReg>();
  ...
  EXPECT_EQ(P.parse(2, Bad, "tool", &ErrOS), nullptr);
  ...
  EXPECT_EQ(P2.parse(2, HelpArgv, "tool", &ErrOS,
                     /*VersionString=*/{}, &HS),
            nullptr);
...
//===----------------------------------------------------------------------===//
// OptionsRegistry::parse() -- thin, no-OptionsContext single-registry parse.
//===----------------------------------------------------------------------===//

TEST(CommandLineV2, RegistryParseSucceeds) {
  auto [Opts, Err] =
      parseSingle<&CmpReg>({"tool", "-cmp-width=9", "-cmp-flag"});
  ASSERT_TRUE(Err.empty()) << Err;
  ASSERT_TRUE(Opts.has_value());
  EXPECT_EQ(Opts->get<&CmpWidth>(), 9);
  EXPECT_TRUE(Opts->get<&CmpFlag>());
}

TEST(CommandLineV2, RegistryParseUnknownOptionError) {
  auto [Opts, Err] = parseSingle<&CmpReg>({"tool", "-notexist"});
  EXPECT_FALSE(Opts.has_value());
  EXPECT_FALSE(Err.empty());
  EXPECT_NE(Err.find("Unknown command line argument"), std::string::npos);
}

TEST(CommandLineV2, RegistryParseHelpDoesNotExit) {
  // With an error stream supplied, --help must return std::nullopt rather
  // than terminate the process -- mirrors ShortHelpWithErrsDoesNotExit for
  // OptionParser, but exercised against OptionsRegistry::parse() directly.
  std::string Help, Err;
  raw_string_ostream HS(Help), ES(Err);
  const char *argv[] = {"tool", "--help"};
  auto Opts = CmpReg.parse(2, argv, "tool", &ES, /*VersionString=*/{}, &HS);
  EXPECT_FALSE(Opts.has_value());
  EXPECT_NE(Help.find("--cmp-width"), std::string::npos);
  EXPECT_TRUE(Err.empty()) << Err;
}

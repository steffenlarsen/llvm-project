...
template <typename FnT> constexpr bool isNullFnPtr(FnT F) { return F == nullptr; }
...
static_assert(!isNullFnPtr(StaticInfoFor<&OptOptim>.PrintEnumVal));
...
TEST(CommandLineV2, CompiledParserHonoursOnErrorReturn) {
  ...
  EXPECT_EQ(CP.parse(2, Bad, "tool", &ErrOS), nullptr);
  // And a good parse still works after a bad one.
  const char *Good[] = {"tool", "--cmp-width=5"};
  auto Ctx = CP.parse(2, Good, "tool", &ErrOS);
  ASSERT_NE(Ctx, nullptr);

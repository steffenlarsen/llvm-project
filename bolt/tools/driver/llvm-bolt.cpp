...
struct BoltDriverOptions {
  std::string InputFilename;
  std::string InputFilename2;
  std::string InputDataFilename;
  std::string LogFile;
  std::string InputDataFilename2;
};
...
void perf2boltMode(int argc, char **argv, BoltDriverOptions &DriverOpts) {
  ...
  DriverOpts.InputDataFilename = Opts->get<&BoltData>();
  DriverOpts.LogFile = Opts->get<&BoltLogFile>();
  DriverOpts.InputDataFilename2 = Opts->get<&BoltData2>();
  ...
  if (!DriverOpts.InputDataFilename.empty()) {
...
void boltDiffMode(int argc, char **argv, BoltDriverOptions &DriverOpts) {
  ...
  DriverOpts.LogFile = Opts->get<&BoltLogFile>();
  DriverOpts.InputDataFilename2 = Opts->get<&BoltData2>();
  if (DriverOpts.InputDataFilename2.empty()) {
  ...
  if (DriverOpts.InputFilename2.empty()) {
    errs() << ToolName << ": expected second binary name.\n";
    exit(1);
  }
  if (DriverOpts.InputFilename.empty()) {
...
void boltMode(int argc, char **argv, BoltDriverOptions &DriverOpts) {
...
#include "bolt/Core/TargetConfig.def"
  ...
  BoltDriverOptions DriverOpts;
  if (llvm::sys::path::filename(ToolName).starts_with("perf2bolt"))
    perf2boltMode(argc, argv, DriverOpts);
  else if (llvm::sys::path::filename(ToolName).starts_with("llvm-boltdiff"))
    boltDiffMode(argc, argv, DriverOpts);
  else
    boltMode(argc, argv, DriverOpts);

  if (!sys::fs::exists(DriverOpts.InputFilename))
    report_error(DriverOpts.InputFilename, errc::no_such_file_or_directory);
  ...
  if (!DriverOpts.LogFile.empty()) {
    std::error_code LogEC;
    LogFileStream = std::make_unique<raw_fd_ostream>(
        DriverOpts.LogFile, LogEC, sys::fs::OpenFlags::OF_None);
    if (LogEC) {
  ...
  if (!DiffOnly) {
    Expected<OwningBinary<Binary>> BinaryOrErr =
        createBinary(DriverOpts.InputFilename);
    if (Error E = BinaryOrErr.takeError())
      report_error(DriverOpts.InputFilename, std::move(E));
    ...
    if (auto *e = dyn_cast<ELFObjectFileBase>(&Binary)) {
      ...
      if (!DriverOpts.InputDataFilename.empty()) {
        if (Error E = RI.setProfile(DriverOpts.InputDataFilename))
          report_error(DriverOpts.InputDataFilename, std::move(E));
      }
      ...
      if (!DriverOpts.InputDataFilename.empty())
        if (Error E = MachORI.setProfile(DriverOpts.InputDataFilename))
          report_error(DriverOpts.InputDataFilename, std::move(E));

      MachORI.run();
    } else {
      report_error(DriverOpts.InputFilename, object_error::invalid_file_type);
    }
  ...
  Expected<OwningBinary<Binary>> BinaryOrErr2 =
      createBinary(DriverOpts.InputFilename2);
  if (Error E = BinaryOrErr1.takeError())
    report_error(DriverOpts.InputFilename, std::move(E));
  if (Error E = BinaryOrErr2.takeError())
    report_error(DriverOpts.InputFilename2, std::move(E));
  ...
  if (auto *ELFObj1 = dyn_cast<ELFObjectFileBase>(&Binary1)) {
    if (auto *ELFObj2 = dyn_cast<ELFObjectFileBase>(&Binary2)) {
      ...
      if (Error E = RI1.setProfile(DriverOpts.InputDataFilename))
        report_error(DriverOpts.InputDataFilename, std::move(E));
      auto RI2OrErr =
      ...
      if (Error E = RI2.setProfile(DriverOpts.InputDataFilename2))
        report_error(DriverOpts.InputDataFilename2, std::move(E));
      outs() << "BOLT-DIFF: *** Analyzing binary 1: "
             << DriverOpts.InputFilename << "\n";
      outs() << "BOLT-DIFF: *** Binary 1 fdata:     "
             << DriverOpts.InputDataFilename << "\n";
      if (Error E = RI1.run())
        report_error(DriverOpts.InputFilename, std::move(E));
      outs() << "BOLT-DIFF: *** Analyzing binary 2: "
             << DriverOpts.InputFilename2 << "\n";
      outs() << "BOLT-DIFF: *** Binary 2 fdata:     "
             << DriverOpts.InputDataFilename2 << "\n";
      if (Error E = RI2.run())
        report_error(DriverOpts.InputFilename2, std::move(E));
      RI1.compare(RI2);
    } else {
      report_error(DriverOpts.InputFilename2, object_error::invalid_file_type);
    }
  } else {
    report_error(DriverOpts.InputFilename, object_error::invalid_file_type);
  }

...
LogicalResult SerializeGPUModuleBase::optimizeModule(llvm::Module &module,
  ...
  //   - memory(argmem: readwrite): non-kernel device helpers (matches what
  //     the optimizer would eventually infer, but gives it upfront so the
  //     inliner and unroller see correct memory effects from the start)
  if (optLevel > 0) {
    for (llvm::Function &F : module) {
      if (F.isDeclaration())
        continue;
      ...
      if (F.getCallingConv() == llvm::CallingConv::AMDGPU_KERNEL) {
        ...
        // Non-kernel device helpers only access memory through arguments.
        if (F.getMemoryEffects() == llvm::MemoryEffects::unknown()) {
          F.setMemoryEffects(

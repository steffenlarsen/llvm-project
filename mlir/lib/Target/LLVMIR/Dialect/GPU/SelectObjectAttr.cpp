...
static LogicalResult embedBinaryROCm(StringRef moduleName,
  ...
  // -------------------------------------------------------------------------
  // 4. Collect kernel names.
  //    - "defined" kernels have code in this binary (from gpu.kernels attr or
  //      the object's kernel table). These are registered with the runtime.
  //    - "launched" kernels are referenced by gpu.launch_func but may be
  //      defined in another TU. These get handle globals for launch but are
  //      NOT registered (to avoid registering against an empty fat binary).
  // -------------------------------------------------------------------------
  SmallVector<std::string> definedKernels;
  SmallVector<std::string> kernelNames;

  // Collect kernels defined in this binary.
  if (binaryOp) {
    if (auto kernelsAttr =
            binaryOp->getAttrOfType<mlir::ArrayAttr>("gpu.kernels")) {
      for (auto attr : kernelsAttr) {
        if (auto strAttr = mlir::dyn_cast<mlir::StringAttr>(attr)) {
          std::string kName = strAttr.getValue().str();
          if (!llvm::is_contained(definedKernels, kName))
            definedKernels.push_back(kName);
  ...
  // Collect kernels launched from this TU (may include cross-TU references).
  if (binaryOp) {
  ...
  // __hipRegisterVar: void(ptr handle, ptr hostVar, ptr deviceAddr,
  //                        ptr deviceName, i32 ext, usize size,
  //                        i32 constant, i32 normalized)
  auto *sizeTy = builder.getIntNTy(module.getDataLayout().getPointerSizeInBits());
  FunctionCallee registerVar = module.getOrInsertFunction(
      "__hipRegisterVar",
      FunctionType::get(voidTy,
                        {ptrTy, ptrTy, ptrTy, ptrTy, i32Ty, sizeTy, i32Ty,
                         i32Ty},
                        false));
  ...
  // Only register kernels that have code in this binary. Cross-TU kernels
  // (launched but not defined here) are registered by the TU that defines them.
  ...
  // Register constant variables (from "gpu.constant_vars" attribute).
  if (binaryOp) {
    if (auto constVarsAttr =
            binaryOp->getAttrOfType<mlir::ArrayAttr>("gpu.constant_vars")) {
      Constant *zero32 = ConstantInt::get(i32Ty, 0);
      Constant *one32 = ConstantInt::get(i32Ty, 1);
      for (auto varAttr : constVarsAttr) {
        auto varDict = mlir::dyn_cast<mlir::DictionaryAttr>(varAttr);
        if (!varDict)
          continue;
        auto nameAttr = varDict.getAs<mlir::StringAttr>("name");
        auto sizeAttr = varDict.getAs<mlir::IntegerAttr>("size");
        if (!nameAttr || !sizeAttr)
          continue;
        std::string varName = nameAttr.getValue().str();
        uint64_t varSize = sizeAttr.getInt();

        // Get the existing host-side shadow global. It was created by
        // OffloadPromoteConstantArgs with a scalar type (e.g., double).
        // getGlobalVariable does exact-type lookup, so use
        // getNamedGlobal which is name-only.
        auto *hostVar = module.getNamedGlobal(varName);
        if (!hostVar) {
          auto *arrTy = ArrayType::get(
              IntegerType::get(module.getContext(), 8), varSize);
          hostVar = new GlobalVariable(
              module, arrTy, /*isConstant=*/false,
              GlobalValue::InternalLinkage,
              ConstantAggregateZero::get(arrTy), varName);
        }

        // Create the device name string.
        Constant *nameInit =
            ConstantDataArray::getString(module.getContext(), varName);
        auto *nameGV = new GlobalVariable(
            module, nameInit->getType(), /*isConstant=*/true,
            GlobalValue::InternalLinkage, nameInit,
            moduleName + "_" + varName + "_name");

        Constant *varSizeConst = ConstantInt::get(sizeTy, varSize);
        Value *hostVarPtr = hostVar;
        if (hostVar->getType()->getPointerAddressSpace() != 0)
          hostVarPtr = builder.CreateAddrSpaceCast(hostVar, ptrTy);
        builder.CreateCall(registerVar,
                           {regHandle, hostVarPtr, nameGV, nameGV,
                            zero32, varSizeConst, one32, zero32});
      }
    }
  }
  ...
  for (auto [kName, kHandle] : llvm::zip(kernelNames, kernelHandles)) {
    ...
    for (unsigned i = 0; i < numArgs; ++i) {
      ...
      // hipLaunchKernel reads each kernel argument via memcpy from
      // args[i], so args[i] must point *to* the argument value.
      //
      // An aggregate passed indirectly arrives as a pointer that already
      // points to the data, so it goes into args[i] as-is.  A parameter
      // whose own type is a pointer (e.g. a `float *` kernel argument) is
      // different: the pointer is the value, so it needs a slot of its own
      // and args[i] must hold that slot's address.  Storing it directly
      // would make the runtime copy from the pointed-to memory -- which for
      // a device pointer means dereferencing a device address on the host.
      //
      // llvm.byval marks the indirect-aggregate case; the SysV rewrite in
      // CIR's LowerToLLVM attaches it to every MEMORY-class aggregate.
      if (stubFn->getArg(i)->hasByValAttr()) {

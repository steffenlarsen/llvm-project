...
buildDepMap(gpu::GPUModuleOp inputMod,
  ...
  inputMod.walk([&](gpu::GPUFuncOp fn) {
    // Uses *within* fn -- the symbols its body references.  Note this is the
    // one-operand overload; getSymbolUses(fn, inputMod) would instead give the
    // uses *of* fn, which is the opposite edge direction.
...
struct SplitSingleSourcePass
  ...
  void runOnOperation() override {
    ...
    // Locate the device module(s) to split.  Preferred form: every gpu.module
    // the producer marked with kOffloadDeviceModuleAttrName.  Selecting by
    // attribute keeps this pass independent of the producer's naming scheme --
    // the CIR offload pipeline names its device module @offload_device_module
    // in single-source mode but @offload_device_module_<arch> after module
    // merging, and a name-based lookup silently does nothing when it guesses
    // wrong.  The gpu-module-name option remains as an explicit override for
    // standalone use and tests.
    SmallVector<gpu::GPUModuleOp> inputMods;
    if (gpuModuleNameWasSet()) {
      if (auto named = module.lookupSymbol<gpu::GPUModuleOp>(gpuModuleName))
        inputMods.push_back(named);
    } else {
      module.walk([&](gpu::GPUModuleOp m) {
        // A deferred module is itself a device module, but it is this pass's
        // own output and must not be split again.
        if (m->hasAttr(gpu::kOffloadDeviceModuleAttrName) &&
            !m->hasAttr(gpu::kOffloadDeferredModuleAttrName))
          inputMods.push_back(m);
      });
    }

    for (gpu::GPUModuleOp inputMod : inputMods)
      splitModule(module, inputMod);
  }

  /// True when the caller explicitly requested a specific module name rather
  /// than relying on attribute-based discovery.
  bool gpuModuleNameWasSet() const { return !gpuModuleName.empty(); }

  void splitModule(ModuleOp module, gpu::GPUModuleOp inputMod) {
    ...
    if (doNone) {
      ...
      // A kernel reached only through a function pointer has no
      // gpu.launch_func, so launches alone under-approximate liveness.  The
      // producer marks such kernels with "gpu.address_taken"; they must be
      // kept even though nothing here launches them directly.
      inputMod.walk([&](gpu::GPUFuncOp fn) {
        if (fn.isKernel() && fn->hasAttr("gpu.address_taken"))
          primaryKernels.insert(fn.getNameAttr());
      });
    ...
    // ------------------------------------------------------------------ //
    // The primary module keeps the input's name so existing gpu.launch_func
    // references stay valid.  The deferred module derives its name from it
    // unless one was given explicitly.
    StringRef primaryName = inputMod.getName();
    std::string deferredName = deferredGpuModuleName.empty()
                                   ? (primaryName + "_deferred").str()
                                   : deferredGpuModuleName;

    builder.setInsertionPointAfter(inputMod);
    auto gpuModulePrimary =
        gpu::GPUModuleOp::create(builder, inputMod.getLoc(), primaryName);
    gpu::GPUModuleOp gpuModuleDeferred;
    if (doSplit)
      gpuModuleDeferred =
          gpu::GPUModuleOp::create(builder, inputMod.getLoc(), deferredName);

    // Both outputs are device modules for the same target as the input, so
    // carry the input's discardable attributes (offload marker, target, and
    // any #gpu.target array already attached) across.  Without this the
    // deferred module looks untargeted and is dropped before code generation.
    for (NamedAttribute attr : inputMod->getDiscardableAttrs()) {
      gpuModulePrimary->setAttr(attr.getName(), attr.getValue());
      if (gpuModuleDeferred)
        gpuModuleDeferred->setAttr(attr.getName(), attr.getValue());
    }
    if (auto targets = inputMod.getTargetsAttr()) {
      gpuModulePrimary.setTargetsAttr(targets);
      if (gpuModuleDeferred)
        gpuModuleDeferred.setTargetsAttr(targets);
    }
    if (gpuModuleDeferred)
      gpuModuleDeferred->setAttr(gpu::kOffloadDeferredModuleAttrName,
                                 UnitAttr::get(ctx));
    ...
    // ------------------------------------------------------------------ //
    // Step 2b: Carry over everything Steps 1b and 2 did not classify.
    //
    // Besides gpu.func and cir.global, the input module can hold external
    // declarations that device bodies call -- LLVM intrinsics such as
    // @llvm.amdgcn.implicitarg.ptr, and device-library entry points such as
    // the __ockl_* printf helpers.  The input module is erased below, so any
    // op left behind becomes a dangling symbol reference that only surfaces
    // much later, as a null-callee crash during LLVM IR translation.
    //
    // Declarations are cloned into the deferred module as well: it is a
    // standalone device image and its kernels may call the same externals.
    // Definitions are moved to the primary module only, matching how
    // non-replicable resources are handled above.
    // ------------------------------------------------------------------ //
    for (Operation &op :
         llvm::make_early_inc_range(inputMod.getBody()->getOperations())) {
      if (op.hasTrait<OpTrait::IsTerminator>())
        continue;
      bool isDeclaration = op.getNumRegions() == 0 ||
                           llvm::all_of(op.getRegions(),
                                        [](Region &r) { return r.empty(); });
      if (isDeclaration && doSplit && gpuModuleDeferred) {
        OpBuilder::InsertionGuard guard(builder);
        builder.setInsertionPointToStart(gpuModuleDeferred.getBody());
        builder.clone(op);
      }
      op.moveBefore(gpuModulePrimary.getBody(),
                    gpuModulePrimary.getBody()->begin());
    }

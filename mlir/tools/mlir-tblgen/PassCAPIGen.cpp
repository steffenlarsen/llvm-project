...
static bool emitCAPIHeader(const RecordKeeper &records, raw_ostream &os) {
  ...
  os << "MLIR_CAPI_EXPORTED void mlirRegister" << groupNamePassCAPI
     << "Passes(void);\n\n";
  for (const auto *def : records.getAllDerivedDefinitions("PassBase")) {
    Pass pass(def);
    StringRef defName = pass.getDef()->getName();
    os << formatv(passDecl, groupNamePassCAPI, defName);
  }
  os << fileFooter;
  return false;
}
...
static bool emitCAPIImpl(const RecordKeeper &records, raw_ostream &os) {
  ...
  os << formatv(passGroupRegistrationCode, groupNamePassCAPI);
  ...
  for (const auto *def : records.getAllDerivedDefinitions("PassBase")) {
    ...
    os << formatv(passCreateDef, groupNamePassCAPI, defName, constructorCall);
  }
  return false;
}
